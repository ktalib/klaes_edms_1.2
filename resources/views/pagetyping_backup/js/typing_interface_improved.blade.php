<!-- IMPROVED: Complete Page Typing Interface JavaScript with Enhanced PDF Support -->
<script src="https://cdnjs.cloudflare.com/ajax/libs/pdf.js/3.11.174/pdf.min.js"></script>
<script>
console.log('🔧 LOADING IMPROVED TYPING INTERFACE WITH ENHANCED PDF SUPPORT...');
...
window.initializeTypingInterface = function() {
    @if(isset($selectedFileIndexing))
        console.log('🚀 Initializing IMPROVED typing interface for file:', {{ $selectedFileIndexing->id }});
        ...
        // Load PDF document - IMPROVED IMPLEMENTATION WITH ENHANCED ERROR HANDLING
        async function loadPdfDocument(document) {
            console.log('📄 STARTING IMPROVED PDF DOCUMENT LOAD:', document.file_url);
            try {
                ...
                // Extract pages with improved error handling
                currentPdfPages = [];
                let successfulPages = 0;
                let failedPages = 0;
                
                console.log(`🔄 Starting IMPROVED page extraction for ${numPages} pages...`);
                ...
                for (let pageNum = 1; pageNum <= numPages; pageNum++) {
                    ...
                    try {
                        ...
                        const scales = [1.5, 1.0, 2.0, 0.75, 0.5];
                        let viewport = null;
                        ...
                        // Adjust viewport if we had to limit dimensions
                        if (actualWidth !== viewport.width || actualHeight !== viewport.height) {
                            const scaleX = actualWidth / viewport.width;
                            const scaleY = actualHeight / viewport.height;
                            const adjustedScale = Math.min(scaleX, scaleY) * workingScale;
                            viewport = page.getViewport({ scale: adjustedScale });
                            console.log(`🔧 Adjusted viewport to ${viewport.width}x${viewport.height} with scale ${adjustedScale}`);
                        }
                        
                        console.log(`🖼️ Rendering page ${pageNum} to canvas...`);
                        const renderContext = {
                        ...
                        // Try to create a placeholder page
                        try {
                            console.log(`🔄 Creating placeholder for failed page ${pageNum}...`);
                            const canvas = document.createElement('canvas');
                            ...
                            context.fillText(`Page ${pageNum}`, canvas.width/2, canvas.height/2 - 20);
                            
                            context.font = '16px Arial';
                            context.fillText('(Rendering failed)', canvas.width/2, canvas.height/2 + 10);
                            context.fillText('Click to view original', canvas.width/2, canvas.height/2 + 35);
                            ...
                            console.log(`⚠️ Added placeholder for page ${pageNum}`);
                            successfulPages++;
                ...
                console.log(`📊 IMPROVED Page extraction summary:`, {
                    totalPages: numPages,
                ...
                if (currentPdfPages.length === 0) {
                    throw new Error(`No pages could be extracted from PDF. Total pages: ${numPages}, Successful: ${successfulPages}, Failed: ${failedPages}. This might be due to a corrupted PDF or browser compatibility issues.`);
                }
                
                console.log(`🎉 Successfully extracted ${currentPdfPages.length} pages (${successfulPages} rendered, ${failedPages} placeholders)`);
        ...
        function renderPdfPage(pageIndex) {
            ...
            if (documentViewer) {
                ...
                // Add click handler for placeholder pages
                if (pdfPage.isPlaceholder && pdfPage.originalUrl) {
                    content += `
                            <div class="absolute inset-0 flex items-center justify-center bg-black bg-opacity-50 opacity-0 hover:opacity-100 transition-opacity cursor-pointer" onclick="window.open('${pdfPage.originalUrl}', '_blank')">
                                <div class="text-white text-center">
                                    <i data-lucide="external-link" class="h-8 w-8 mx-auto mb-2"></i>
                                    <p>Click to view original PDF</p>
                                </div>
                            </div>
                    `;
                }
...
console.log('✅ IMPROVED typing interface loaded and ready!');
</script>