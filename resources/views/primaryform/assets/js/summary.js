...
function collectFormData() {
    ...
    const phoneInputs = document.querySelectorAll('input[name="phone_number[]"], input[name="phone"], input[name="phone_alternate"]');
    formData.phoneNumber = Array.from(phoneInputs)
        .map(input => (input.value || '').trim())
        .filter(value => value !== '')
        .join(', ');
    const ownerEmailInput = document.querySelector('input[name="owner_email"]') || document.querySelector('input[name="email"]');
    formData.email = ownerEmailInput ? ownerEmailInput.value : '';
