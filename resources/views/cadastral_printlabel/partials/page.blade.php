<div class="p-6">
    <div class="container mx-auto py-6 space-y-6">
        ...
        <div class="w-full">
            ...
            <div id="files-tab" class="tab-content active mt-6">
                <div class="bg-white rounded-lg border">
                    <div class="p-6 border-b bg-gradient-to-r from-slate-50 to-white">
                        ...
                        <div class="mt-4 grid gap-3 sm:grid-cols-2 sm:items-start">
                            <div class="flex flex-col gap-2 text-xs text-slate-600">
                                <span class="inline-flex items-center gap-2 self-start rounded-full border border-blue-100 bg-blue-50 px-3 py-1 font-medium text-blue-700">
                                    <i data-lucide="check-square" class="h-3.5 w-3.5"></i>
                                    Manual selection
                                </span>
                                <span class="leading-relaxed">
                                    Pick individual files or switch on <span class="font-semibold text-slate-700">Batch Mode</span> below to pull the next grouping slice automatically.
                                </span>
                            </div>
                            <div class="flex flex-wrap items-center gap-2 text-xs text-slate-600 sm:justify-end">
                                @if($showOnlyST)
                                    <span class="inline-flex items-center gap-2 rounded-full border border-emerald-100 bg-emerald-50 px-3 py-1 font-medium text-emerald-700">
                                        <i data-lucide="layers" class="h-3.5 w-3.5"></i>
                                        Sectional Titling mode
                                    </span>
                    ...
                    <div class="p-6">
                        <div class="mb-6" id="batchModeWrapper">
                            <div id="batchModeCard" class="rounded-lg border border-slate-200 bg-slate-50 px-4 py-4 transition-colors">
                                <div class="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
                                    <label for="batchMode" class="flex cursor-pointer items-start gap-3">
                                        <input
                                            type="checkbox"
                                            id="batchMode"
                                        ...
                                        <span class="flex flex-col">
                                            <span class="text-sm font-semibold text-slate-700">Batch Mode</span>
                                            <span class="text-xs text-slate-500">Pull unassigned grouping rows by registry batch and load them into the preview in one go.</span>
                                        </span>
                                    </label>
                                    <div class="text-xs text-slate-500 sm:text-right">
                                        Select this when printing sequential grouping batches. We automatically limit each fetch to 100 items.
                                    </div>
                                </div>

                                <div id="batchControls" class="hidden mt-4 space-y-6">
                                    <div class="grid grid-cols-1 gap-4 lg:grid-cols-5">
                                        <div class="flex flex-col">
                                            <span class="text-xs font-semibold uppercase tracking-wide text-slate-600">Start Row</span>
                                            <input
                                                type="number"
                                                id="batchStart"
                                                min="1"
                                                ...
                                                value="1"
                                            />
                                            <span class="mt-1 text-xs text-slate-500">Row position to begin fetching from the grouping table.</span>
                                        </div>
                                        <div class="flex flex-col">
                                            <span class="text-xs font-semibold uppercase tracking-wide text-slate-600">Registry</span>
                                            <select
                                                id="registrySelect"
                                                class="mt-1 w-full rounded-md border border-gray-300 px-3 py-2 text-sm focus:border-blue-500 focus:outline-none focus:ring-2 focus:ring-blue-200"
                                            >
                                                <option value="1" selected>Registry 1</option>
                                                <option value="2">Registry 2</option>
                                                <option value="3">Registry 3</option>
                                            </select>
                                            <span class="mt-1 text-xs text-slate-500">Filter batches by primary registry.</span>
                                        </div>
                                        <div class="flex flex-col" id="registryYearWrapper">
                                            <span class="text-xs font-semibold uppercase tracking-wide text-slate-600">Record Year</span>
                                            <input
                                                type="number"
                                                id="registryYearInput"
                                                min="1900"
                                                max="{{ now()->year + 1 }}"
                                                placeholder="1981"
                                                class="mt-1 w-full rounded-md border border-gray-300 px-3 py-2 text-sm focus:border-blue-500 focus:outline-none focus:ring-2 focus:ring-blue-200"
                                            />
                                            <span class="mt-1 text-xs text-slate-500">Optional filter for grouping rows by <code>year</code>.</span>
                                        </div>
                                        <div class="flex flex-col" id="registryBatchWrapper">
                                            <span class="text-xs font-semibold uppercase tracking-wide text-slate-600">Registry Batch Numbers</span>
                                            <input
                                                type="text"
                                                id="registryBatchInput"
                                                placeholder="Enter values e.g. 1,2,3"
                                                class="mt-1 w-full rounded-md border border-gray-300 px-3 py-2 text-sm focus:border-blue-500 focus:outline-none focus:ring-2 focus:ring-blue-200"
                                                autocomplete="off"
                                            />
                                            <span class="mt-1 text-xs text-slate-500">
                                                Separate entries with commas or press Enter to create tags. Matches the <code>registry_batch_no</code> stored on grouping records.
                                            </span>
                                            <div
                                                id="registryBatchChips"
                                                class="mt-2 flex flex-wrap gap-1.5 text-[11px]"
                                                aria-live="polite"
                                            ></div>
                                        </div>
                                        <div class="flex flex-col">
                                            <span class="text-xs font-semibold uppercase tracking-wide text-slate-600">Range</span>
                                            <div class="mt-1 w-full rounded-md border border-gray-300 px-3 py-2 text-sm bg-gray-50 text-slate-700 font-semibold">
                                                1 – 100 (Per batch)
                                            </div>
                                            <span class="mt-1 text-xs text-slate-500">We pull 100 rows per registry batch; multiple entries multiply the total selection.</span>
                                        </div>
                                    </div>

                                    <div id="registryOverrideCard" class="rounded-lg border border-amber-200 bg-amber-50 px-4 py-4 text-xs text-amber-800">
                                    ...
                                    <div class="grid grid-cols-1 gap-4 md:grid-cols-4">
                                        <div class="flex flex-col">
                                            <span class="text-xs font-semibold uppercase tracking-wide text-slate-600">Rack 1</span>
                                            <select
                                        ...
                                        <div class="flex flex-col">
                                            <span class="text-xs font-semibold uppercase tracking-wide text-slate-600">Rack 2 (Backup)</span>
                                            <select
                                        ...
                                        <div class="flex flex-col">
                                            ...
                                            />
                                            <span class="mt-1 text-xs text-slate-500">Auto-generated from rack &amp; shelf selections.</span>
                                        </div>
                                    </div>
                                    ...
                                    <div class="grid grid-cols-1 gap-4 md:grid-cols-2">
                                        ...
                                        <div id="noSequenceWrapper" class="hidden rounded-lg border border-dashed border-blue-100 bg-white px-3 py-3 text-xs text-blue-700">
                                            <label for="noSequenceToggle" class="flex cursor-pointer items-start gap-3">
                                                <input
                                                    type="checkbox"
                                                    id="noSequenceToggle"
                                                    class="mt-1 rounded border-blue-300 text-blue-600 focus:ring-blue-500"
                                                />
                                                <span>
                                                    <span class="block font-semibold uppercase tracking-wide text-[11px] text-blue-800">No Sequence</span>
                                                    <span>Duplicate the selected rack & shelf for the entered count instead of incrementing (e.g., 50 x A1).</span>
                                                </span>
                                            </label>
                                        </div>
                                    </div>

                                        <div id="shelfModeNote" class="hidden rounded-lg border border-blue-100 bg-blue-50 px-4 py-3 text-xs text-blue-800">
                                            <div class="font-semibold uppercase tracking-wide text-[11px] text-blue-900">Shelf labels only</div>
                                            <p class="mt-1 leading-relaxed text-blue-900/80">
                                            After choosing a rack and shelf, use <span class="font-semibold">Load &amp; Print</span> to immediately print the selected label sequence (Full Label only).
                                            </p>
                                        </div>
                        ...
                        <div id="fileList" class="rounded-md border">
                            <div class="p-3 bg-gray-50 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                                ...
                                <div class="flex items-center gap-2 text-sm text-gray-500" id="selectionStatusWrapper">
                                    ...
                                    <span id="selectionStatus">0 of 100 selected</span>
                                </div>
                            </div>
            ...
            @include('cadastral_printlabel.partials.preview')
        </div>
    </div>
</div>
