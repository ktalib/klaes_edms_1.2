                ...
                @php
                    ...
                    if (is_array($ownerNames) && count($ownerNames) > 0) {
                        if (count($ownerNames) === 1) {
                            ...
                            echo '<span onclick="showAllOwners(' .
                                htmlspecialchars(json_encode($ownerNames), ENT_QUOTES, 'UTF-8') .
...
@else
<!-- This empty div will be populated by JavaScript when an application is selected -->
@endif

@if(!isset($application))
<script>
    // This function is only needed when the header is included without an application 
