...
<head>
  ...
  <title>Temporary File Number Application - {{ $record->file_number ?? '' }}</title>
  <style>
...
<div class="page">
  ...
  {{-- BODY --}}
  <div class="para">
    At page <strong>{{ $record->page ?? '1' }}</strong> is an application for temporary file No.
    <strong>{{ $record->file_number ?? 'LKN/RES/......./.......' }}</strong>
    over Plot No. <strong>{{ $record->plot_number ?? '...............' }}</strong>
    part of <strong>{{ $record->layout_plan_no ?? '...............' }}</strong>
    situated at <strong>{{ $record->location ?? '...............' }}</strong>
    by the applicant herein; <strong>{{ $record->applicant_name ?? '...............' }}</strong>.
  </div>

  <div class="para">
    The application was forwarded to Cadastral Department for verification and available records at Cadastral Department vide page <span class="pg-ref">{{ $record->page_2 ?? '' }}</span> indicate that, as per their record Plot No. <strong>{{ $record->plot_number ?? '...............' }}</strong> part of <strong>{{ $record->layout_plan_no ?? '...............' }}</strong> within <strong>{{ $record->street_name ?? '...............' }}</strong> is open. In addition, the file was referred to Deed Department and vide the report at page <span class="pg-ref">{{ $record->page_3 ?? '' }}</span> the title No. <strong>{{ $record->file_number ?? '...............' }}</strong> is at Letter of Grant Stage.
  </div>
