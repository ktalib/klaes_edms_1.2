...
<head>
  ...
  <title>Ministry of Works - {{ $record->file_number ?? '' }}</title>
  <style>
...
<div class="page">
  ...
  <div class="para">
    The applicant was allocated GP No. <strong>{{ $record->plot_number ?? '...............' }}</strong>
    by the Federal Ministry of Works and Housing under the monetization policy of Government.
    The applicant has settled all appropriate payments as per clearance letter at page <span class="pg-ref">{{ $record->page_2 ?? '' }}</span>
    where it is confirmed that, the beneficiary has paid the sum of
    <strong>&#x20A6;{{ isset($record->premium) ? number_format($record->premium, 2) : '.................' }}</strong>
    ({{ $record->premium_words ?? '.....................................' }}) (as purchase price) with receipt No. <span class="pg-ref">{{ $record->page_3 ?? '' }}</span> copy attached a, b, c.
    ...
    ({{ $record->preparation_fees_words ?? '.....................................' }}) being payment for 5% purchasing price of GP vide page <span class="pg-ref">{{ $record->page_4 ?? '' }}</span> and triplicate receipt at back cover.
  </div>
