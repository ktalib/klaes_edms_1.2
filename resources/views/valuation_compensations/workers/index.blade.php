                    ...
                    <tbody id="worker-pool-body">
                        ...
                        <tr class="hover:bg-slate-50/50 transition-colors border-b border-slate-50 last:border-0">
                            ...
                            <td class="px-6 py-5 text-right">
                                <button type="button" onclick="removeWorker({{ $worker->id }}, '{{ $worker->user->first_name }} {{ $worker->user->last_name }}')" 
        ...
        Swal.fire({
            ...
            if (result.isConfirmed) {
                $.ajax({
                    ...
                    error: function() {
                        Swal.fire('Error!', 'Could not remove worker.', 'error');
                    }
                });
            }
        });
