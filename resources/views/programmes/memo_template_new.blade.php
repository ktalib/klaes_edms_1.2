...
<body class="py-4" data-date="{{ $generatedAtDisplay }}" data-user="{{ $generatedBy }}">
    ...
    <div class="memo-container" data-date="{{ $generatedAtDisplay }}" data-user="{{ $generatedBy }}">
        ...
        <div class="text-gray-700 mb-6">
            ...
            <p class="mb-4 font-semibold whitespace-nowrap">In view of the above, you may kindly wish to recommend the following for approval of the <br> Honourable Commissioner.</p>
    ...
    <script>
        ...
        async function exportToWord() {
            try {
                ...
                const memoData = {
                    memoNo: @json($memo->memo_no ?? '-'),
                    pageNo: @json($memo->page_no ?? '-'),
                    landUse: @json(strtoupper(trim($memo->land_use ?? '')) ?: '-'),
                    legacyReferenceLabel: @json($legacyReferenceLabel ?? '-'),
                    legacyReferenceValue: @json($legacyReferenceValue ?? '-'),
                    propertyLocation: @json(!empty($formattedPropertyLocation) ? $formattedPropertyLocation : (!empty($formattedFallbackLocation) ? $formattedFallbackLocation : '-')),
                    applicantName: @json($formatTitle($applicantName) ?? '-'),
                    prevailingLandUse: @json(strtoupper(trim($memo->prevailing_land_use ?? '')) ?: '-'),
                    siteMeasurement: @json($siteMeasurementSummaryDisplay ?? '-'),
                    sharedUtilities: @json($sharedUtilitiesSummaryDisplay ?? '-'),
                    sitePlanNo: @json($memo->site_plan_no ?? '-'),
                    arcDesignPageNo: @json($memo->arc_design_page_no ?? '-'),
                    termYears: @json($memo->term_years ?? ($totalYears ?? '-')),
                    commencementDate: @json($memo->commencement_date ? date('d/m/Y', strtotime($memo->commencement_date)) : ($memo->approval_date ? date('d/m/Y', strtotime($memo->approval_date)) : '-')),
                    residualYears: @json($memo->residual_years ?? ($residualYears ?? '-')),
                    propertyPlotNo: @json($memo->property_plot_no ?? '-'),
                    primaryApplicationNo: @json($primaryApplicationNumberDisplay ?? '-'),
                    directorRank: @json($memo->director_rank ?? 'Director Sectional Titling'),
                ...
                // Logos removed from Word document export
        ...
        // Inline onclick handlers need a global reference.
