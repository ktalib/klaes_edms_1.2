...
<body class="py-4" data-date="{{ $generatedAtDisplay }}" data-user="{{ $generatedBy }}">
    ...
    <div class="memo-container">
        ...
        <div class="text-gray-700 mb-6">
            <p class="mb-2">At page <span class="data-value">{{ $memo->page_no ?? ($landAdmin->page_no ?? '-') }}</span> is an application for <span class="data-value">{{ strtoupper($landUseDisplay ?? '-') }}</span> sectional title in respect of a property,  covered by <span class="data-value">{{ $allocationEntityDisplay }}</span> reference No <span class="data-value">{{ $allocationRefNoDisplay }}</span> situated at <span class="data-value">{{ ucwords(strtolower($propertyLocationText)) }}</span> in the name of <span class="data-value">{{ ucwords(strtolower($applicantDisplayName)) }}</span>.</p>
            
            <p class="mb-2">As well as change of name for the unit owner as per attached on the application.</p>
            
            <p class="mb-2">The application was referred to One stop shop for planning, engineering as well as architectural views. The planners recommended the application because it is feasible, and the unit meets the minimum requirements for <span class="data-value">{{ strtoupper ($landUseDisplay ?? '-') }}</span> title with Unit area measurement: <span class="data-value">{{ $existingSiteMeasurementSummary }}</span> and <span class="data-value">{{ $sharedUtilitiesDisplay }}</span> as shared utilities/common areas.</p>
            
            <p class="mb-2">However, this recommendation is based on recommended site plan at page <span class="data-value">{{ $memo->site_plan_no ?? ($landAdmin->site_plan_page_no ?? '-') }}</span> and architectural design at page <span class="data-value">{{ $memo->arc_design_page_no ?? ($landAdmin->arc_design_page_no ?? '-') }}</span> with measurements of approved dimensions contained Therein respectively.</p>

            <p class="mb-2">Meanwhile, the title was granted for <span class="data-value">{{ strtoupper(trim($memo->prevailing_land_use ?? '')) ?: '-' }}</span> purposes for a term of <span class="data-value">{{ $memo->term_years ?? ($totalYears ?? '-') }}</span> years commencing from <span class="data-value">{{ $memo->commencement_date ? date('d/m/Y', strtotime($memo->commencement_date)) : ($memo->approval_date ? date('d/m/Y', strtotime($memo->approval_date)) : '18/10/2025') }}</span> and has a residual term of <span class="data-value">{{ $memo->residual_years ?? ($residualYears ?? '-') }}</span> years to expire.</p>

            <p class="mb-2 font-semibold">In view of the above, you may kindly wish to recommend the following for approval of the Honourable Commissioner.</p>
            
            <ol class="list-decimal pl-5 mb-4">
                <li class="mb-1">Consider and approve the application for Sectional Titling over a standalone <span class="data-value">{{ strtoupper($suaApplication->land_use ?? $landUseDisplay) }}</span> unit situated at <span class="data-value">{{ ucwords(strtolower($propertyLocationText)) }}</span> covered by ownership document from <span class="data-value">{{ $allocationEntityDisplay }}</span> with reference No. <span class="data-value">{{ $allocationRefNoDisplay }}</span> in Favour of <span class="data-value">{{ ucwords(strtolower($applicantDisplayName)) }}</span>.</li>

                <li class="mb-1">Consider and approve the change of name to the new owner <span class="data-value">{{ ucwords(strtolower($applicantDisplayName)) }}</span> of the stand-alone unit.</li>

                <li class="mb-1">Consider And Approve the Cancellation of the Allocation Slip From <span class="data-value">{{ $allocationEntityDisplay }}</span> To Pave Way For A New Unit(S) of Stand-Alone Sectional Title With File No <span class="data-value">{{ $suaApplication->fileno ??  '-' }}</span>.</li>
            </ol>
