...
<body class="py-4" data-date="{{ $generatedAtDisplay }}" data-user="{{ $generatedBy }}">
    ...
    <div class="no-print flex justify-center space-x-4 mb-4">
        ...
        <button id="downloadDocx" class="bg-green-600 hover:bg-green-700 text-white font-semibold py-2 px-6 rounded-lg shadow-md transition duration-200 flex items-center">
            <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5 mr-2" fill="none" viewBox="0 0 24 24" stroke="currentColor">
    ...
    <script>
        ...
        document.getElementById('downloadDocx').addEventListener('click', function() {
            ...
            // Prepare buyers list data
            const buyersData = @json(isset($buyersList) && count($buyersList) > 0 ? $buyersList : (isset($memo->buyers) && is_array($memo->buyers) && count($memo->buyers) > 0 ? $memo->buyers : []));
                ...
                sections: [{
                    ...
                    children: [
                        ...
                        new Paragraph({
                            ...
                            children: [
                                new TextRun({ text: @json($memo->memo_no ?? 'Memo/2025/01'), bold: true }),
                                new TextRun({ text: "\nPERMANENT SECRETARY", bold: true })
                            ]
                        }),
                        
                        // Horizontal line
                        new Paragraph({ text: "_________________________________________________________", spacing: { after: 200 } }),
                        
                        // Main content with dynamic data
                        new Paragraph({ 
                            text: `Kindly find page ${@json($memo->page_no ?? '-')} is an application for ${@json(strtoupper(trim($memo->land_use ?? '')) ?: '-')} sectional titles in respect of fragmentation of the property, covered by ${@json($legacyReferenceLabel)} ${@json($legacyReferenceValue)} situated at ${@json($formattedPropertyLocation ?? $formattedFallbackLocation ?? '-')} in the name of ${@json($formatTitle($applicantName) ?? '-')}.`,
                            spacing: { after: 200 }
                        }),
                        
                        new Paragraph({ text: "As well as change of name to various unit owners as per attached on the application.", spacing: { after: 200 } }),
                        
                        new Paragraph({ 
                            text: `The application was referred to Physical Planning Department for planning, engineering as well as architectural views. The planners recommended the application because it is feasible, and meets the minimum requirements for ${@json(strtoupper(trim($memo->land_use ?? '')) ?: '-')} titles. Moreover, the proposal is accessible and conforms with the existing ${@json(strtoupper(trim($memo->prevailing_land_use ?? '')) ?: '-')} development in the area with Existing site area measurements: ${@json($siteMeasurementSummaryDisplay)} and ${@json($sharedUtilitiesSummaryDisplay)} as shared Utilities / common areas.`,
                            spacing: { after: 200 }
                        }),
                        
                        new Paragraph({ 
                            text: `However, this recommendation is based on recommended site plan at page ${@json($memo->site_plan_no ?? '-')} and architectural design at ${@json($memo->arc_design_page_no ?? '-')} with measurements of approved dimensions contained Therein respectively.`,
                            spacing: { after: 200 }
                        }),
                        
                        new Paragraph({ 
                            text: `Meanwhile, the title was granted for ${@json(strtoupper(trim($memo->prevailing_land_use ?? '')) ?: '-')} purposes for a term of ${@json($memo->term_years ?? ($totalYears ?? '-'))} years commencing from ${@json($memo->commencement_date ? date('d/m/Y', strtotime($memo->commencement_date)) : ($memo->approval_date ? date('d/m/Y', strtotime($memo->approval_date)) : '-'))} and has a residual term of ${@json($memo->residual_years ?? ($residualYears ?? '-'))} years to expire.`,
                            spacing: { after: 200 }
                        }),
                        
                        new Paragraph({ 
                            children: [new TextRun({ text: "In view of the above, you may kindly wish to recommend the following for approval of the Honourable Commissioner.", bold: true })],
                            spacing: { after: 200 }
                        }),
                        
                        // Numbered list
                        new Paragraph({ 
                            text: `1. Consider and approve the application for Sectional Titling over plot ${@json($memo->property_plot_no ?? '-')} situated at ${@json($formattedListAddress ?? '-')} covered by ${@json($legacyReferenceLabel)} ${@json($legacyReferenceValue)} in Favour of ${@json($formatTitle($applicantName) ?? '-')}.`,
                            spacing: { after: 100 }
                        }),
                        new Paragraph({ 
                        ...
                        new Paragraph({ 
                            text: `3. Consider and approve the Cancellation of Old Statutory File No mentioned above, to pave way for Sectional titles with Primary Application File No ${@json($primaryApplicationNumberDisplay)} and issuance of new Sectional Titles to the new owners as per buyers list attached on the next page.`,
                            spacing: { after: 300 }
                        }),
                        
                        // Signatures section
                        new Paragraph({ text: "Name: _________________________      Rank: _________________________", spacing: { after: 100 } }),
                        new Paragraph({ text: "Sign: _________________________      Date: _______________", spacing: { after: 100 } }),
                        new Paragraph({ text: "Sign: _________________________", spacing: { after: 100 } }),
                        new Paragraph({ text: @json($memo->director_rank ?? 'Director Sectional Titling'), spacing: { after: 100 } }),
                        new Paragraph({ text: "Date: _______________", spacing: { after: 300 } }),
                        
                        // Permanent Secretary
                        new Paragraph({ 
                            children: [new TextRun({ text: "PERMANENT SECRETARY", bold: true })],
                            spacing: { after: 200 }
                        }),
                        new Paragraph({ text: "The application is hereby recommended for your kind approval, please. _________________________", spacing: { after: 100 } }),
                        new Paragraph({ text: "Date: _______________ 2025.                                                  Permanent Secretary", spacing: { after: 300 } }),
                        
                        // Honourable Commissioner
                        new Paragraph({ 
                            children: [new TextRun({ text: "HONOURABLE COMMISSIONER", bold: true })],
                            spacing: { after: 200 }
                        }),
                        new Paragraph({ text: "The application is hereby APPROVED/NOT APPROVED. _________________________", spacing: { after: 100 } }),
                        new Paragraph({ text: "Date: _______________ 2025.                                                  Honourable Commissioner", spacing: { after: 400 } }),
                        ...
                        // Buyers List
                        new Paragraph({ 
                            children: [new TextRun({ text: "BUYERS LIST", bold: true, size: 24 })],
                            spacing: { after: 200 }
                        }),
                        ...
                        new Table({
                            ...
                            columnWidths: [1000, 4000, 2000, 2000],
                            rows: [
                                // Header row
                                new TableRow({
                                    children: [
                                        new TableCell({ children: [new Paragraph({ text: "SN", bold: true })] }),
                                        new TableCell({ children: [new Paragraph({ text: "BUYER NAME", bold: true })] }),
                                        new TableCell({ children: [new Paragraph({ text: "UNIT NUMBER", bold: true })] }),
                                        new TableCell({ children: [new Paragraph({ text: "MEASUREMENT (SQM)", bold: true })] })
                                    ]
                                }),
                                // Data rows
                                ...buyersData.map((buyer, index) => {
                                    const buyerName = buyer.buyer_name || 
                                                    (buyer.buyer_title && buyer.buyer_name ? 
                                                     `${buyer.buyer_title} ${buyer.buyer_name}` : 
                                                     (buyer.buyer_name || '-'));
                                    const unitNo = buyer.unit_no || buyer.unit_number || '-';
                                    const measurement = buyer.measurement || buyer.plot_size || '-';
                                    
                                    return new TableRow({
                                        children: [
                                            new TableCell({ children: [new Paragraph(`${index + 1}.`)] }),
                                            new TableCell({ children: [new Paragraph(buyerName)] }),
                                            new TableCell({ children: [new Paragraph(unitNo)] }),
                        ...
                        // Footer
                        new Paragraph({ 
                            text: `Generated on: ${@json($generatedAtDisplay)} by ${@json($generatedBy)}`,
            ...
            Packer.toBlob(doc).then(blob => {
                saveAs(blob, "Sectional_Titling_Memo_{{ $memo->memo_no ?? '2025' }}.docx");
            }).catch(error => {
