...
document.addEventListener('DOMContentLoaded', function() {
    ...
    let state = {
        ...
        dcivPrefix: 'DCIV', dcivBatchNo: '',
    };

    const PRINT_TEMPLATE_URL = '/dciv-printlabel/print-template';
    const API = {
        prefixes:        '/dciv-printlabel/api/prefixes',
        prefixNextRange: '/dciv-printlabel/api/prefix-next-range',
        files:           '/dciv-printlabel/api/files',
        rackStatus:      '/dciv-printlabel/api/rack-label/status',
        createBatch:     '/dciv-printlabel/api/batch',
        batches:         '/dciv-printlabel/api/batches',
        batchBase:       '/dciv-printlabel/api/batch/',
    };
    ...
    function computePreparationSignature() {
        ...
        return JSON.stringify({selectedIds:ids,fullLabel:state.fullLabel,selectedTemplate:state.selectedTemplate,orientation:state.orientation,dcivPrefix:state.dcivPrefix,dcivBatchNo:state.dcivBatchNo});
    }
    ...
    // ---- DCIV file loading ----
    async function loadDcivFiles() {
        var prefix=state.dcivPrefix;
        var batchNo=state.dcivBatchNo;
        if(!prefix){showError('Please select a prefix.');return;}
        if(!batchNo||parseInt(batchNo)<1){showError('Please enter a valid batch number.');return;}
        showLoading('Loading '+prefix+' files...');
        try{
            var params=new URLSearchParams({prefix:prefix,sys_batch_no:batchNo,search:state.searchTerm||''});
            ...
            hideLoading();
            if(data.data&&data.data.batch_already_used){
                Swal.fire({icon:'warning',title:'Batch Already Used',html:'Batch <strong>#'+batchNo+'</strong> for '+prefix+' has already been generated.<br>You cannot generate labels for this batch again.',confirmButtonColor:'#3b82f6'});
                state.selectedFiles=[];renderFileList();updateCounts();updateSelectAllCheckbox();
                var gb=document.getElementById('generateBatchBtn');if(gb){gb.disabled=true;gb.classList.add('opacity-50','cursor-not-allowed');}
                var cs=document.getElementById('continueToSettingsBtn');if(cs){cs.disabled=true;cs.classList.add('opacity-50','cursor-not-allowed');}
                return;
            }
            if(files.length===0)showError('No records found for '+prefix+' batch #'+batchNo+'.');
            else showSuccess('Loaded '+files.length+' file'+(files.length===1?'':'s')+' from '+prefix+' batch #'+batchNo+'.');
    ...
    async function persistBatchForPrinting() {
        ...
        var payload={prefix:state.dcivPrefix,sys_batch_no:parseInt(state.dcivBatchNo,10),file_ids:sel.map(function(r){return Number(r.id);}),
    ...
    function renderFileList() {
        ...
        if(state.availableFiles.length===0){
            el.innerHTML='<div class="p-8 text-center text-gray-500"><div class="mb-2"><i data-lucide="file-text" class="h-8 w-8 mx-auto text-gray-400"></i></div><p>No files loaded yet.</p><p class="text-xs text-gray-400 mt-1">Select a prefix, enter a batch number and click Load Records.</p></div>';
    ...
    function openPrintWindowWithPayload(payload) {
        var payloadKey='dciv-printlabel-'+Date.now();
    ...
    async function printLabels(event) {
        ...
        payload.autoPrint=true;payload.notice='Generated via DCIV Print Labels';
    ...
    // Auto-suggest next batch number on page load for the default prefix
    function autoSuggestNextBatch() {
        fetch(API.prefixNextRange+'?prefix='+encodeURIComponent(state.dcivPrefix)).then(function(r){return r.json();}).then(function(d){
            if(d.success&&d.data){var bi=document.getElementById('dcivBatchNoInput');if(bi&&!bi.value){bi.value=d.data.next_batch_no;state.dcivBatchNo=d.data.next_batch_no.toString();}}
        }).catch(function(e){console.warn('Auto-suggest error:',e);});
    }
    autoSuggestNextBatch();

    var dcivPrefixSelect=document.getElementById('dcivPrefixSelect');
    if(dcivPrefixSelect){dcivPrefixSelect.addEventListener('change',function(){
        state.dcivPrefix=this.value;
        // Re-suggest batch number for new prefix
        var bi=document.getElementById('dcivBatchNoInput');if(bi){bi.value='';state.dcivBatchNo='';}
        autoSuggestNextBatch();
    });}
    var dcivBatchNoInput=document.getElementById('dcivBatchNoInput');
    if(dcivBatchNoInput){dcivBatchNoInput.addEventListener('change',function(){state.dcivBatchNo=this.value;});}
    ...
    document.getElementById('generateBatchBtn').addEventListener('click',function(){
        ...
        loadDcivFiles().finally(function(){btn.disabled=false;btn.classList.remove('opacity-50','cursor-not-allowed');});
    });
    document.getElementById('resetBtn').addEventListener('click',function(){
        resetPreparedState();state.selectedFiles=[];state.copies=1;state.dcivBatchNo='';state.dcivPrefix='DCIV';
        ...
        if(dcivPrefixSelect)dcivPrefixSelect.value='DCIV';
        if(dcivBatchNoInput)dcivBatchNoInput.value='';
