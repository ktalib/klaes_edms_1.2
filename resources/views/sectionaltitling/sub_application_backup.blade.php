...
</script<script src="{{ asset('resources/views/sectionaltitling/sub_application_fix.js') }}"></script>

<script>
// ULTIMATE STEP NAVIGATION FIX - FINAL VERSION
(function() {
    'use strict';
    
    console.log('🚀 ULTIMATE FINAL step navigation fix loading...');
    
    // Wait for everything to load
    function initializeUltimateNavigation() {
        console.log('🔧 Initializing ULTIMATE navigation...');
        ...
        function validateStep1() {
            ...
            const errors = [];
            if (!applicantType) errors.push('Please select an applicant type');
            if (!schemeNo || schemeNo.trim() === '') errors.push('Please enter scheme number');
        ...
        function validateStep3() {
            // Allow navigation to step 4 without document validation for now
            return [];
        }
        ...
        // ULTIMATE navigation function
        window.goToStep = function(stepNumber, skipValidation = false) {
            console.log(`🚀 ULTIMATE navigation to step ${stepNumber}, skip: ${skipValidation}`);
            ...
            console.log(`Current: ${currentStep}, Target: ${stepNumber}`);
            ...
            if (stepNumber > currentStep && !skipValidation) {
                let errors = [];
                switch (currentStep) {
            ...
            // ULTIMATE hide all steps
            const allSteps = document.querySelectorAll('.form-section');
            console.log(`Hiding ${allSteps.length} steps`);
            
            allSteps.forEach((step, index) => {
                step.classList.remove('active-tab');
                // Use the most aggressive hiding possible
                step.style.cssText = 'display: none !important; visibility: hidden !important; opacity: 0 !important; position: absolute !important; left: -99999px !important; top: -99999px !important; z-index: -9999 !important; height: 0 !important; width: 0 !important; overflow: hidden !important;';
                step.setAttribute('hidden', 'true');
                step.setAttribute('aria-hidden', 'true');
                console.log(`🔒 ULTIMATE hidden step ${index + 1}`);
            });
            
            // ULTIMATE show target step
            const targetStep = document.getElementById(`step${stepNumber}`);
            if (targetStep) {
                // Use the most aggressive showing possible
                targetStep.classList.add('active-tab');
                targetStep.style.cssText = 'display: block !important; visibility: visible !important; opacity: 1 !important; position: static !important; left: auto !important; top: auto !important; z-index: auto !important; height: auto !important; width: auto !important; overflow: visible !important;';
                targetStep.removeAttribute('hidden');
                targetStep.removeAttribute('aria-hidden');
                
                // Force multiple layout recalculations
                targetStep.offsetHeight;
                targetStep.scrollHeight;
                targetStep.clientHeight;
                targetStep.getBoundingClientRect();
                
                console.log(`✅ ULTIMATE showed step ${stepNumber}:`, {
                    offsetHeight: targetStep.offsetHeight,
                    scrollHeight: targetStep.scrollHeight,
                    display: window.getComputedStyle(targetStep).display,
                    visibility: window.getComputedStyle(targetStep).visibility,
                    opacity: window.getComputedStyle(targetStep).opacity
                });
                
                // SPECIAL ULTIMATE handling for step 3
                if (stepNumber === 3) {
                    console.log('🔧 ULTIMATE step 3 fixes...');
                    ...
                    targetStep.style.transform = 'translateZ(0)';
                    targetStep.offsetHeight; // Force reflow
                    targetStep.style.transform = '';
                    
                    // Emergency refresh
                    setTimeout(() => {
                        if (targetStep.offsetHeight === 0) {
                            console.log('🚨 EMERGENCY step 3 refresh');
                            const content = targetStep.innerHTML;
                            targetStep.innerHTML = '';
                            targetStep.innerHTML = content;
                            targetStep.style.cssText = 'display: block !important; visibility: visible !important; opacity: 1 !important; position: static !important;';
                        }
                    }, 100);
                }
                ...
                circles.forEach((circle, index) => {
                    circle.classList.remove('active-tab', 'inactive-tab');
                    circle.classList.add(index + 1 === stepNumber ? 'active-tab' : 'inactive-tab');
                });
        ...
        function attachUltimateListeners() {
            console.log('🔗 Attaching ULTIMATE listeners...');
            ...
            document.querySelectorAll('.step-circle').forEach((circle, index) => {
                ...
                newCircle.addEventListener('click', function(e) {
                    e.preventDefault();
                    e.stopPropagation();
                    console.log(`🔘 Circle ${stepNum} clicked`);
                    window.goToStep(stepNum, true);
                });
            ...
            // Next buttons
            ['nextStep1', 'nextStep2', 'nextStep3'].forEach((id, index) => {
                ...
                if (element) {
                    ...
                    newElement.addEventListener('click', function(e) {
                        ...
                        window.goToStep(index + 2, false);
                    });
                }
            });
            
            // Back buttons
            ['backStep2', 'backStep3', 'backStep4'].forEach((id, index) => {
                const element = document.getElementById(id);
                if (element) {
                    const newElement = element.cloneNode(true);
                    if (element.parentNode) {
        ...
        setTimeout(() => {
            ...
            if (step1) {
                ...
                step1.style.cssText = 'display: block !important; visibility: visible !important; opacity: 1 !important; position: static !important;';
                step1.removeAttribute('hidden');
                console.log('✅ Step 1 ULTIMATE visible');
            }
        }, 100);
        
        console.log('🎉 ULTIMATE navigation initialized!');
    }
    
    // Multiple initialization attempts
    if (document.readyState === 'loading') {
        document.addEventListener('DOMContentLoaded', () => setTimeout(initializeUltimateNavigation, 500));
    } else {
        setTimeout(initializeUltimateNavigation, 500);
    }
    
    window.addEventListener('load', () => setTimeout(initializeUltimateNavigation, 1000));
    
    // Final fallback
    setTimeout(initializeUltimateNavigation, 3000);
})();
</script>
