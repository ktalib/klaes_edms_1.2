...
<div id="surveyModal" class="fixed inset-0 z-50 hidden bg-black bg-opacity-50 flex items-center justify-center">
    <div class="bg-white rounded-lg shadow-lg w-full max-w-md">
        ...
        <div class="px-4 py-4 bg-gray-50">
            <form   method="POST" action="{{ route('mother-applications.store') }}">
                @csrf
                {{-- Hidden input to store the application ID --}} 
                <input type="hidden" id="survey-application-id"   value="" disabled>
              

                {{-- Applicant Name and File No (Readonly) - Assuming these might still be useful --}}
                <div class="grid grid-cols-2 gap-4 mb-4" style="display: ndone;">
                    <div>
                        <label class="block text-sm font-medium text-gray-700 mb-1">FileNo</label>
                        {{-- Changed ID to be unique for the survey modal --}}
                    ...
                    <div>
                        ...
                        <input type="text" name="application_id" class="w-full px-3 py-2 border border-gray-300 rounded-md bg-gray-100"  id="survey-file-no">
                    </div>
                </div>
                ...
                <div class="mb-4">
                    {{-- TODO: Implement data-bs-toggle/target functionality or replace with Tailwind equivalent modal trigger --}}
                ...
                <div class="mt-5 pt-4 border-t border-gray-200">
                    <div class="flex justify-end space-x-3">
                        ...
                        {{-- TODO: Implement showDepartmentConfirmation JS function --}}
                        <button type="button" class="px-3 py-1.5 bg-green-100 text-green-800 rounded-md hover:bg-green-200 flex items-center shadow-sm text-sm"
                            onclick="showDepartmentConfirmation('ok')">
                            OK
                            <i data-lucide="check-circle" class="w-4 h-4 ml-1 text-green-600"></i>
                        </button>
                        <button type="button" class="px-3 py-1.5 bg-gray-100 text-gray-800 rounded-md hover:bg-gray-200 flex items-center shadow-sm text-sm"
...
<div id="deedsModal" class="fixed inset-0 z-50 hidden bg-black bg-opacity-50 flex items-center justify-center">
    <div class="bg-white rounded-lg shadow-lg w-full max-w-md">
        ...
        <div class="px-4 py-4 bg-gray-50">
            <form  method="POST" action="{{ route('mother-applications.storeDeeds') }}">
                @csrf
           
                <input type="hidden" id="deeds-application-id" name="application_id" value="">
                
                <div class="grid gap-4 mb-3">
                    
                         
                        <input type="hidden" class="w-full px-3 py-2 border border-gray-300 rounded-md" name="application_id"  id="applicant-name">
               
                    <div>
                        <label class="block text-sm font-medium text-gray-700 mb-1">File No</label>
                        <input type="text" class="w-full px-3 py-2 border border-gray-300 rounded-md" id="deedfileno-display"readonly  >
                    </div>
                </div>
...
@include('sectionaltitling.action_modals.survey_plan')
<script>
    //OtherApprovals
    ...
    function openSurveyModal() {
        ...
        fetch(`/gisedms/mother-applications/${currentApplicationId}`)
            .then(response => {
    ...
    async function fetchFileNo(applicationId) {
        try {
            ...
            const response = await fetch(`/mother-applications/${applicationId}`);
