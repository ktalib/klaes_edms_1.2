{{--
    Plot Merger memo to the Permanent Secretary / Honourable Commissioner.
...
@php
    ...
    // Where the land is. The district, then the LGA - a merger names one place for
    // the parcels it brings together, and district alone printed blank without it.
...
<head>
    ...
    <title>Merger Recommendation - KLAES GIS</title>
    <style>
        :root {
            --page-color: #fdf6e3;
            --accent-red: #cc0000;
    ...
    <div class="a4-page">
        ...
        <div class="main-container">
            ...
            <div class="body-paragraph">
                At Page 1 is an application for <span class="bold-caps">PLOT MERGER</span> over properties situated <span class="bold-caps">{{ $situated }}</span> covered by Certificate of occupancy no. <span class="bold-caps">{{ Str::upper($record->file_no) }}</span> in favour of <span class="bold-caps">{{ Str::upper($record->applicant_name) }}.</span>
            </div>
            <div class="body-paragraph">
                The application seeks to merge <span class="bold-caps">{{ $record->num_plots }}</span> adjacent plots{{ $sizes['phrase'] }} into a single title. Verification at Cadastral and Deeds Departments confirms that the titles are free from encumbrances and suitable for the proposed merger.
            </div>
            <div class="body-paragraph">
                In view of the above, you may kindly wish to recommend this application for Merger to <span class="bold-caps">Honourable Commissioner</span> for approval of:
            </div>
            <div class="point-block">
                a.) Merger of {{ $record->num_plots }} plots{{ $sizes['total_phrase'] }} as described in the application into a unified title in favor of {{ Str::upper($record->applicant_name) }}.
            </div>
