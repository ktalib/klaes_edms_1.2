{{--
    Plot Subdivision memo to the Permanent Secretary / Honourable Commissioner.
...
@php
    ...
    // Where the land is. plot_subdivision_applications has no `location` column - the sheet
    // read one for years and silently got null - so the district answers, then the LGA.
...
<head>
    ...
    <title>Subdivision Recommendation - KLAES GIS</title>
    <style>
        :root {
            --page-color: #fdf6e3;
            --accent-red: #cc0000;
    ...
    <div class="a4-page">
        ...
        <div class="main-container">
            ...
            <div class="body-paragraph">
                At Page 1 is an application for <span class="bold-caps">PLOT SUBDIVISION</span> over a piece of land situated <span class="bold-caps">{{ $situated }}</span> covered by Certificate of occupancy no. <span class="bold-caps">{{ Str::upper($record->file_no) }}</span> in favour of <span class="bold-caps">{{ Str::upper($record->applicant_name) }}.</span>
            </div>
            <div class="body-paragraph">
                The application seeks to subdivide the aforementioned property into <span class="bold-caps">{{ $record->num_plots }}</span> plots{{ $sizes['phrase'] }}. Verification at Cadastral and Deeds Departments confirms that the title is free from encumbrances and suitable for the proposed subdivision.
            </div>
            <div class="body-paragraph">
                In view of the above, you may kindly wish to recommend this application for Subdivision to <span class="bold-caps">Honourable Commissioner</span> for approval of:
            </div>
            <div class="point-block">
                a.) Subdivision of plot no. <span class="bold-caps">{{ Str::upper($record->plot_no) }}</span> into {{ $record->num_plots }} parcels{{ $sizes['phrase'] }} as per the attached plan in favor of {{ Str::upper($record->applicant_name) }}.
            </div>
