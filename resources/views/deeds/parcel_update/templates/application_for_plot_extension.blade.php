...
<head>
  ...
  <title>Plot Extension Application - {{ $record->file_no ?? '' }}</title>
  <style>
...
<div class="page">
  ...
  {{-- BODY --}}
  <div class="para">
    At page 1 is an application for Plot Extension in respect of the property covered by
    <strong>{{ $record->file_no ?? '...............' }}</strong>
    situated at <strong>{{ $record->house_no ? $record->house_no.', ' : '' }}{{ $record->street_name ?? '...............' }}{{ $record->district ? ', '.$record->district : '' }}, {{ $record->lga ?? '...............' }}</strong>
    by the title holder: <strong>{{ $record->applicant_name ?? '...............' }}</strong>.
  </div>

  <div class="para">
    At page &nbsp;&nbsp;&nbsp;&nbsp; for the planning views, where the State Planning Authority at page &nbsp;&nbsp;&nbsp;&nbsp; recommended the application for Extension in view of the fact that, the site is adequately accessible and conforms with the existing land use of the area.
  </div>
  ...
  <div class="para">
    The assessed value of the extended plot is
    <strong>&#x20A6;{{ isset($record->land_value) ? number_format($record->land_value, 2) : '.................' }}</strong>
    and all applicable fees have been paid by the applicant vide receipts at back cover.
  </div>
  ...
  <div class="rec-block">
    <div class="para">
      I. <strong>CONSIDER AND APPROVE</strong> the extension in respect of
      <strong>{{ $record->file_no ?? '...............' }}</strong>
      over the piece of land situated at
      {{ $record->street_name ?? '...............' }}{{ $record->district ? ', '.$record->district : '' }}
