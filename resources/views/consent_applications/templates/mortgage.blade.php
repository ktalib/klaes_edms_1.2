...
<head>
    ...
    <title>Consent to Mortgage Property -{{ $application->file_number }}</title>
    ...
    <style>
        ...
        @media print {
            ...
            body {
                ...
                /* Without this the default 8px body margin alone pushes the
                   297mm sheet onto a second page. */
                margin: 0;
                padding: 0;
            }
...
<body class="print:bg-white">
    ...
    <div class="page-sheet">
        ...
        <div class="our-ref" data-fit-ref>{{ $displayFileNumber }}</div>
        ...
        <div class="mt-2 leading-tight recipient">
            <span class="name">{{ $displayApplicantName }}</span><br>
        ...
        <div class="subject">
            ...
            <div><span data-fit-line>CERTIFICATE OF OCCUPANCY: <span
                        class="json-data">{{ $displayFileNumber }}</span></span></div>
        </div>

        <div class="mt-0 leading-tight text-md text-justify">
            By virtue of the powers conferred upon the Governor of Kano state by the provisions of section 21 and 22 of
            ...
            @else
                <ol class="inline list-none p-0 m-0">
                    <li class="inline">(1) <span class="json-data">{{ strtoupper($application->party_name) }}</span> of <span class="json-data">{{ rtrim($application->party_address, '.') }}</span></li>
                    @foreach($application->additional_parties as $index => $party)
                        <li class="inline">and ({{ $index + 2 }}) <span class="json-data">{{ strtoupper($party['name']) }}</span> of <span class="json-data">{{ rtrim($party['address'], '.') }}</span></li>
                    @endforeach
                </ol>.
            @endif
            For a consideration of <span class="json-data">{{ $application->consideration_words }} (₦{{ $application->consideration }})</span>
            only. This is subject to the submission of
    ...
    <script>
        ...
        // "Date:" field — merged multi-property file numbers can be long.
