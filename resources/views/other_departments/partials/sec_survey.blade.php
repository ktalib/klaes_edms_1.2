    ...
    <div id="initial-tab" class="tab-content {{ request()->has('survey') || (!request()->has('deeds') && !request()->has('lands')) ? 'active' : '' }}">
      ...
      <div class="border border-gray-200 rounded-lg p-4 bg-gray-50">
        ...
        <div class="flex gap-2">
          <a href="{{route('other_departments.survey_secondary')}}" class="flex items-center px-3 py-1 text-xs border border-gray-300 rounded-md bg-white hover:bg-gray-50">
     ...
     fetch(`{{ route('survey.get', '') }}/${applicationId}`)
      ...
      .then(data => {
       if (data.success) {
        ...
        // No record found, leave form enabled for new entry
      ...
      .catch(error => {
       ...
       // If error, enable form fields so user can try to create a new record
       toggleFormFields(true);
      });
  }
