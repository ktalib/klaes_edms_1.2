  ...
  <div class="modal-content modal-center-y lg:max-w-[1100px]" style="width: 90%; max-width: 800px;">
    ...
    <div id="batchSearchSection" class="batch-filter-section available-instruments-section">
        ...
        <div class="flex gap-2">
          ...
          <select id="batchStatusFilter" class="border rounded-md px-3 py-2">
            ...
            @foreach($instrumentTypes ?? [] as $type)
              @php $typeName = is_object($type) ? ($type->name ?? $type) : $type; @endphp
          ...
          <thead class="bg-gray-50">
            <tr>
              <th scope="col" class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">
                <input type="checkbox" class="rounded" id="selectAllAvailable"
                  onchange="toggleSelectAllAvailable(this)">
              </th>
              <th scope="col" class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">
                File No
              </th>
      ...
      <div class="flex justify-end mb-6">
        <button id="addSelectedBtn"
          class="bg-blue-600 text-white px-4 py-2 rounded-md disabled:opacity-50 disabled:cursor-not-allowed"
          onclick="addSelectedToBatch()" disabled>
    ...
    <div id="batchRegistrationForm" class="mt-8 border-t pt-6">
      ...
      <div class="p-4 bg-blue-50 rounded-lg mb-6">
        ...
        <p class="text-sm text-gray-600 mb-4">The system will automatically assign sequential RegNo to each record
          following the pattern: SerialNo/PageNo/VolumeNo</p>

        <div class="bg-white p-3 rounded-md border border-blue-200">
          <p class="text-sm font-medium">Next available Registration number: <span id="batchNextSerialNo"
              class="font-bold">Loading...</span></p>
          <p class="text-xs text-gray-500 mt-1">This is the starting serial number. Each entry will increment
            sequentially.</p>
      ...
      <div class="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
        <div>
          <label for="batchDeedsTime" class="block text-sm font-medium text-gray-700 mb-1">Deeds Time <span
              class="text-red-500">*</span></label>
          <input type="text" id="batchDeedsTime" name="deeds_time"
            class="w-full px-3 py-2 border rounded-md bg-gray-100 text-gray-600" placeholder="eg. 10:30 AM"
            value="{{ date('h:i A') }}" readonly required>
          <div class="text-xs text-gray-500">Format: HH:MM AM/PM</div>
          <div class="text-xs text-red-500 hidden" id="batchDeedsTimeError"></div>
        </div>

        <div>
          <label for="batchDeedsDate" class="block text-sm font-medium text-gray-700 mb-1">Deeds Date <span
              class="text-red-500">*</span></label>
          <input type="date" id="batchDeedsDate" name="deeds_date"
            class="w-full px-3 py-2 border rounded-md bg-gray-100 text-gray-600" value="{{ date('Y-m-d') }}" readonly
            required>
    ...
    <div class="flex justify-end gap-2 mt-6">
      ...
      <button id="batchRegisterButton"
        class="bg-blue-600 text-white px-4 py-2 rounded-md disabled:opacity-50 disabled:cursor-not-allowed"
        onclick="submitBatchRegistration()" disabled>
