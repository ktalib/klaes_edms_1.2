<div   x-data="{ showOther: false, customDistrict: '' }">
<label for="district_name" class="block text-sm font-medium text-gray-700">District Name <span class="text-red-600">*</span></label>
    <select id="district_name" x-model="district_name" class="w-full px-3 py-2 border border-gray-300 rounded-md text-sm transition-all focus:outline-none focus:border-blue-600 focus:ring-2 focus:ring-blue-600/10"
            @change="showOther = district_name === 'other'; if(!showOther) customDistrict = ''"
            :name="showOther ? '' : 'district_name'">
