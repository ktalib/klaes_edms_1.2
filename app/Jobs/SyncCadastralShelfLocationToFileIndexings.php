...
class SyncCadastralShelfLocationToFileIndexings implements ShouldQueue
{
    ...
    /**
     * Execute the job.
     * Syncs shelf_location from cadastral_print_label_batch_items to file_indexings table
     * Silently skips items where file_indexing_id doesn't exist
     */
    public function handle(): array
    {
        Log::info('SyncCadastralShelfLocationToFileIndexings job started', [
            'batch_id' => $this->batchId,
        ]);

        try {
            // Get all items to sync from this batch
            $itemsToSync = CadastralPrintLabelBatchItem::where('batch_id', $this->batchId)
            ...
            if ($totalItems === 0) {
                Log::info('No items to sync for cadastral batch', ['batch_id' => $this->batchId]);
                return [
                    'synced' => 0,
                    'total' => 0,
                    'skipped' => 0,
            ...
            foreach ($itemsToSync->chunk(500) as $chunk) {
                ...
                $validItems = $chunk->filter(function ($item) use ($validIds) {
                    ...
                    Log::debug('Skipping orphaned file_indexing_id during cadastral sync', [
            ...
            Log::info('SyncCadastralShelfLocationToFileIndexings job completed', [
                'batch_id' => $this->batchId,
            ...
            Log::error('SyncCadastralShelfLocationToFileIndexings job failed', [
                'batch_id' => $this->batchId,
                'error' => $e->getMessage(),
                'exception' => $e,
            ]);
