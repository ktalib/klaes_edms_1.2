...
if (!function_exists('workStationModuleMap')) {
    function workStationModuleMap(): array
    {
        $options = config('workstations.options', []);
        $map = [];

        foreach ($options as $option) {
            if (is_array($option)) {
                $label = trim((string) ($option['label'] ?? $option['value'] ?? ''));
                $moduleKey = $option['module_key'] ?? $option['module'] ?? null;
            } else {
                $label = trim((string) $option);
                $moduleKey = null;
            }

            if ($label === '' || empty($moduleKey)) {
                continue;
            }
...
if (!function_exists('parseApplicantOwnersList')) {
    function parseApplicantOwnersList($raw): array
    {
        $names = [];
        ...
        $walk = static function ($payload) use (&$walk, $appendName): void {
            ...
            foreach ($directKeys as $key) {
                if (array_key_exists($key, $payload) && ($payload[$key] !== null && $payload[$key] !== '')) {
                    $appendName($payload[$key]);
                    return;
                }
            }

            if (isset($payload['owners']) && is_array($payload['owners'])) {
                $walk($payload['owners']);
                return;
            }

            $composed = trim(implode(' ', array_filter([
                trim((string) ($payload['title'] ?? $payload['applicant_title'] ?? '')),
                trim((string) ($payload['first_name'] ?? $payload['firstName'] ?? '')),
                trim((string) ($payload['middle_name'] ?? $payload['middleName'] ?? '')),
                trim((string) ($payload['surname'] ?? $payload['last_name'] ?? $payload['lastName'] ?? '')),
            ])));
        ...
        $names = array_values(array_unique(array_filter($names, static function ($value) {
            return $value !== '';
        })));
...
if (!function_exists('formatApplicantDisplayName')) {
    function formatApplicantDisplayName($applicantType, $title = null, $firstName = null, $surname = null, $corporateName = null, $multipleOwnersNames = null): string
    {
        ...
        $corporateTokens = ['corporate', 'coperate', 'company', 'cooperate', 'organisation', 'organization', 'institution', 'firm', 'enterprise', 'limited', 'ltd', 'inc'];
        $multipleTokens = ['multiple', 'joint', 'coowners', 'coowner', 'partners', 'partnership', 'jointowners', 'group'];
        $individualTokens = ['individual', 'person', 'personal', 'private', 'single'];

        $matchesType = static function (array $tokens) use ($typeKey): bool {
            if ($typeKey === '') {
                return false;
            }

            foreach ($tokens as $token) {
                if ($token !== '' && str_contains($typeKey, $token)) {
                    return true;
                }
            }
        ...
        if ($matchesType($corporateTokens) && $corporate !== '') {
            return $corporate;
        }

        if ($matchesType($multipleTokens) && !empty($multipleNames)) {
            return implode(', ', $multipleNames);
        }

        if ($matchesType($individualTokens) && $individual !== '') {
            return $individual;
        }
        ...
        if (is_string($multipleOwnersNames)) {
            $fallback = trim($multipleOwnersNames);
            if ($fallback !== '') {
                return $fallback;
            }
        }
...
if (!function_exists('settingsKeys')) {
    function settingsKeys()
    {
        $settingsKeys = [
            ...
            "pricing_feature" => "on",
            "activity_target_file_indexing" => 100,
            "activity_target_pra" => 120,
            "activity_target_pic" => 150,
            "activity_target_page_typing" => 0,
            "activity_target_scanning" => 0,
            "activity_target_blind_scan" => 0,
        ];

        foreach (workStationModuleMap() as $moduleKey => $label) {
            $settingKey = 'activity_target_' . $moduleKey;
            if (!array_key_exists($settingKey, $settingsKeys)) {
                $settingsKeys[$settingKey] = 0;
            }
        }
 ...
 * 
 * @param string|null $address Raw address from database
...
/**
 * Get Grantor Address based on instrument type and application source
 * 
 * @param string $instrumentType Type of instrument (ST CofO, ST Assignment, etc.)
 * @param object $applicationData Application/Subapplication data
 * @return string Grantor address
 */
if (!function_exists('getGrantorAddress')) {
    function getGrantorAddress($instrumentType, $applicationData)
    {
        // ST CofO: Always "Government House, Kano"
        if ($instrumentType === 'Sectional Titling CofO' || $instrumentType === 'ST CofO') {
        ...
        if ($instrumentType === 'ST Assignment (Transfer of Title)' || $instrumentType === 'ST Assignment') {
            ...
            if ($unitType === 'SUA') {
                ...
                return !empty($allocationSource) ? formatAddress($allocationSource) : '';
            }
        }
...
/**
 * Get Grantee Address based on application source
 * 
 * @param object $applicationData Application/Subapplication data
 * @return string Grantee address
 */
if (!function_exists('getGranteeAddress')) {
    function getGranteeAddress($applicationData)
    {
        // First check for address in subapplications table
        if (isset($applicationData->sub_address) && !empty($applicationData->sub_address)) {
            return formatAddress($applicationData->sub_address);
        }

        // Fall back to address in mother_applications table
        if (isset($applicationData->mother_address) && !empty($applicationData->mother_address)) {
            return formatAddress($applicationData->mother_address);
        }

        // Last resort: check for generic 'address' field
        if (isset($applicationData->address) && !empty($applicationData->address)) {
...
if (!function_exists('normalizeLocationText')) {
    /**
     * Normalize location text by uppercasing parts and ensuring final two segments are joined.
     */
    function normalizeLocationText($text)
    {
        ...
        $parts = array_filter(array_map('trim', explode(',', $normalized)), static function ($part) {
            return $part !== '';
        });
...
if (!function_exists('check_duplicate')) {
    /**
     * Helper to check for duplicate instruments (RofO, CofO, OP, PRA, etc.)
     *
     * @param string $type
     * @param array $params
     * @return array|null
     */
    function check_duplicate($type, array $params)
    {
