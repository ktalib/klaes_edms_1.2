...
class SettingController extends Controller
{
    ...
    public function multipurposeData(Request $request)
    {
        if (!\Gate::check('manage general settings')) {
            return redirect()->back()->with('error', __('Permission denied.'))->with('tab', 'multipurpose_settings');
        }

        $targetFields = [];
        foreach (workStationModuleMap() as $moduleKey => $label) {
            $targetFields['target_' . $moduleKey] = 'activity_target_' . $moduleKey;
        }

        $rules = [
            'site_name' => 'required|string|max:255',
            'site_tagline' => 'nullable|string|max:255',
            'brand_logo' => 'nullable|mimes:png,jpg,jpeg,svg,webp|max:4096',
            'brand_favicon' => 'nullable|mimes:png,jpg,jpeg,svg,webp|max:2048',
            'brand_light_logo' => 'nullable|mimes:png,jpg,jpeg,svg,webp|max:4096',
        ];
        ...
        if ($validator->fails()) {
            $messages = $validator->getMessageBag();
            return redirect()->back()->with('error', $messages->first())->with('tab', 'multipurpose_settings');
        }
        ...
        foreach ($targetFields as $field => $settingKey) {
            \DB::insert(
                'insert into settings (`value`, `name`,`parent_id`) values (?, ?, ?) ON DUPLICATE KEY UPDATE `value` = VALUES(`value`)',
                [
                    (int) $request->input($field, 0),
                    $settingKey,
                    parentId(),
                ]
            );
        ...
        if ($request->hasFile('brand_logo')) {
            ...
            $ownerLogoName = parentId() . '_logo.' . $extension;
            $request->file('brand_logo')->storeAs('upload/logo/', $ownerLogoName);
            \DB::insert(
                'insert into settings (`value`, `name`,`parent_id`) values (?, ?, ?) ON DUPLICATE KEY UPDATE `value` = VALUES(`value`)',
                [
                    $ownerLogoName,
        ...
        if ($request->hasFile('brand_favicon')) {
            $extension = $request->file('brand_favicon')->getClientOriginalExtension();
            $ownerFaviconName = parentId() . '_favicon.' . $extension;
            $request->file('brand_favicon')->storeAs('upload/logo/', $ownerFaviconName);
            \DB::insert(
                'insert into settings (`value`, `name`,`parent_id`) values (?, ?, ?) ON DUPLICATE KEY UPDATE `value` = VALUES(`value`)',
                [
                    $ownerFaviconName,
        ...
        if ($request->hasFile('brand_light_logo')) {
            $extension = $request->file('brand_light_logo')->getClientOriginalExtension();
            $ownerLightLogoName = parentId() . '_light_logo.' . $extension;
            $request->file('brand_light_logo')->storeAs('upload/logo/', $ownerLightLogoName);
            \DB::insert(
                'insert into settings (`value`, `name`,`parent_id`) values (?, ?, ?) ON DUPLICATE KEY UPDATE `value` = VALUES(`value`)',
                [
                    $ownerLightLogoName,
        ...
        return redirect()->back()->with('success', __('Multipurpose settings successfully saved.'))->with('tab', 'multipurpose_settings');
    }
