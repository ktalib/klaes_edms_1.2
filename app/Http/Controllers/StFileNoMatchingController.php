...
use App\Models\Lga;
use App\Models\StShadowFile;
use Illuminate\Http\Request;
use Illuminate\Support\Facades\Auth;
use Illuminate\Support\Facades\DB;
use Illuminate\Support\Facades\Log;

class StFileNoMatchingController extends Controller
{
    /**
     * Display the ST File Number Matching index.
     * Updated for new Shadow File Commissioning (matching-only) architecture.
     */
    public function index(Request $request)
    {
        $PageTitle = 'Match Existing FileNo (ST)';
        $PageDescription = 'Manage ST Shadow Files (SSF)';
        ...
        $query = FileNumber::where('pp_st_matching', 1);
        ...
        $stats = [
            'total' => FileNumber::where('pp_st_matching', 1)->count(),
            'today' => FileNumber::where('pp_st_matching', 1)
                ->whereDate('pp_st_date_matched', date('Y-m-d'))->count(),
            'month' => FileNumber::where('pp_st_matching', 1)
                ->whereNotNull('pp_st_date_matched')
                ->whereMonth('pp_st_date_matched', date('m'))
                ->whereRaw('YEAR(pp_st_date_matched) = ?', [date('Y')])
                ->count(),
        ];

        return view('st_file_no_matching.index', compact(
    ...
    public function store(Request $request)
    {
        ...
        try {
            ...
            if ($existingFile->pp_st_matching == 1) {
                throw new \Exception("File Number: {$fullFileNumber} has already been matched in the ST module.");
            }
            ...
            FileNumber::where('mlsfNo', $fullFileNumber)->update([
                'pp_st_matching' => 1,
                'pp_st_date_matched' => $now->toDateString(),
                'pp_st_time_matched' => $now->toTimeString()
            ]);

            // NOTE: Under the new Shadow File Commissioning architecture,
            // we do NOT create StShadowFile records anymore.
            ...
            Log::error('ST Matching Failed: ' . $e->getMessage());
            return response()->json([
                'success' => false,
    ...
    {
        try {
            $record = StShadowFile::findOrFail($id);
            $data = $record->toArray();
