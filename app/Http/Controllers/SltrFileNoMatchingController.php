...
use App\Models\Lga;
use App\Models\SltrShadowFile;
use Illuminate\Http\Request;
use Illuminate\Support\Facades\Auth;
use Illuminate\Support\Facades\DB;
use Illuminate\Support\Facades\Log;

class SltrFileNoMatchingController extends Controller
{
    /**
     * Display the SLTR File Number Matching index.
     * Updated for new Shadow File Commissioning (matching-only) architecture.
     */
    public function index(Request $request)
    {
        $PageTitle = 'Match Existing FileNo (SLTR)';
        $PageDescription = 'Manage SLTR Shadow Files (SLSF)';
        ...
        $query = FileNumber::where('pp_sltr_matching', 1);
        ...
        $stats = [
            'total' => FileNumber::where('pp_sltr_matching', 1)->count(),
            'today' => FileNumber::where('pp_sltr_matching', 1)
                ->whereDate('pp_sltr_date_matched', date('Y-m-d'))->count(),
            'month' => FileNumber::where('pp_sltr_matching', 1)
                ->whereNotNull('pp_sltr_date_matched')
                ->whereMonth('pp_sltr_date_matched', date('m'))
                ->whereRaw('YEAR(pp_sltr_date_matched) = ?', [date('Y')])
                ->count(),
        ];

        return view('sltr_file_no_matching.index', compact(
    ...
    public function store(Request $request)
    {
        ...
        try {
            ...
            if ($existingFile->pp_sltr_matching == 1) {
                throw new \Exception("File Number: {$fullFileNumber} has already been matched in the SLTR module.");
            }
            ...
            FileNumber::where('mlsfNo', $fullFileNumber)->update([
                'pp_sltr_matching' => 1,
                'pp_sltr_date_matched' => $now->toDateString(),
                'pp_sltr_time_matched' => $now->toTimeString()
            ]);

            // NOTE: Under the new Shadow File Commissioning architecture,
            // we do NOT create SltrShadowFile records anymore.
            ...
            Log::error('SLTR Matching Failed: ' . $e->getMessage());
            return response()->json([
                'success' => false,
    ...
    {
        try {
            $record = SltrShadowFile::findOrFail($id);
            $data = $record->toArray();
    ...
    public function getFileDetails(Request $request)
    {
        try {
            ...
            $data = null;
            $file = DB::connection('sqlsrv')->table('fileNumber')->where('mlsfNo', $fileNumber)->first();
            ...
            if (!$data) return response()->json(['success' => false, 'message' => 'No record found']);
