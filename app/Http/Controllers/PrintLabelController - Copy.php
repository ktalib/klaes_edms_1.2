...
use App\Jobs\SyncShelfLocationToFileIndexings;
...
{ 
    ...
    public function lookupFileNumbers(Request $request)
    {
        ...
        Log::info('printlabel.override.lookup', [
    ...
    public function previewGroupingBatch(Request $request)
    {
        ...
        $cacheKey = sprintf(
            'printlabel:grouping-preview:%s:%d:%d:%d:%d:%s:%s',
...
SQL;
            ...
            Log::info('printlabel.grouping.preview', [
                'registry_batch_list' => $registryBatchValues->all(),
                'start' => $start,
                'per_batch_limit' => $perBatchLimit,
    ...
    public function createBatch(Request $request)
    {
        ...
        Log::info('printlabel.createBatch.requested', [
            'source' => $source,
        ...
        if ($source === 'grouping') {
            ...
            Log::info('printlabel.grouping.cache_ready', [
        ...
        try {
            $result = DB::connection('sqlsrv')->transaction(function () use ($source, $validated, $batchSize, $groupingPreparation) {
                ...
                if ($source === 'grouping') {
                    ...
                    Log::info('printlabel.grouping.before_fileindexing_update', [
                        'batch_id' => $batch->id,
                    ...
                    if (!empty($fileIndexingIdsToUpdate)) {
                        Log::info('printlabel.grouping.skipping_fileindexing_update', [
                        ...
                        // For now, shelf_location is in print_label_batch_items table
                    }
                    ...
                    Log::info('printlabel.grouping.before_batch_insert', [
                        'batch_id' => $batch->id,
                    ...
                    Log::info('printlabel.grouping.batch_items_inserted', [
                        'batch_id' => $batch->id,
                    ...
                    Log::info('printlabel.grouping.processed', [
                        'batch_id' => $batch->id,
                        'registry_batch_no' => $registryBatchNo,
                    ...
                    Log::info('printlabel.fileindexings.batch_items_inserted', [
                        'batch_id' => $batch->id,
                        'items_inserted' => $itemsCreated,
                    ]);
                    $batch->update([
            ...
            Log::info('printlabel.createBatch.completed', [
                'batch_id' => $result['batch']->id,
                'batch_number' => $result['batch']->batch_number,
                'source' => $source,
                'items_created' => $result['items_created'],
            ...
            $labelItems = $batchItems->map(function (PrintLabelBatchItem $item) {
                $qrData = [];
    ...
    private function retrieveAssignedRackLabels(string $registryBatchNo, ?string $registryId = null): Collection
    {
        ...
        if ($normalizedRegistry !== null) {
            ...
            $labels = RackShelfLabel::where('assigned', $targetAssignment)
                ->orderBy('id')
                ->get();
            ...
            $legacyLabels = RackShelfLabel::where('assigned', $normalizedBatch)
                ->orderBy('id')
                ->get();
        ...
        return RackShelfLabel::where(function ($builder) use ($normalizedBatch) {
    ...
    private function insertBatchItemsInChunks(array $items): void
    {
        ...
        foreach (array_chunk($items, $maxPerInsert) as $chunk) {
            PrintLabelBatchItem::insert($chunk);
        }
    }
    ...
    private function resolveRackLabelRecord(
        ...
        $byFullLabel = RackShelfLabel::query();
        if ($lockForUpdate) {
        ...
        $fallbackQuery = RackShelfLabel::query();
        if ($lockForUpdate) {
        ...
        if ($createIfMissing && $rackPrimary !== null && $shelfNumber !== null) {
            ...
            return RackShelfLabel::create($payload);
        }
        ...
        try {
            ...
            if ($batchIds->isNotEmpty()) {
                $yearsByBatchId = DB::connection('sqlsrv')
                    ->table('print_label_batch_items as pbi')
        ...
        try {
            ...
            if ($files->isNotEmpty()) {
                ...
                if ($labelCandidate) {
                    ...
                    $labelRecord = RackShelfLabel::whereRaw('UPPER(LTRIM(RTRIM(full_label))) = ?', [$candidateValue])->first();

                    if (!$labelRecord) {
                        $labelRecord = RackShelfLabel::whereRaw(
    ...
    protected function buildShelfStatistics(?string $registryBatchNo, ?string $registryId = null): ?array
    {
        ...
        $labels = $assignedLabels->map(function (RackShelfLabel $label) use ($capacity) {
        ...
        $nextLabel = RackShelfLabel::whereNull('assigned')
            ->orderBy('id')
            ->first();
