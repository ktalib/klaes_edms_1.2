...
try {
    $result = DB::connection('sqlsrv')->transaction(function() {
        ...
        $newPropId = $service->allocateOrRetrievePropId('RES-2026-2151', 'RES-2026-2151', null, null, ['skip_lookup' => true]);

        // 2. Create the CORRECT OP in PRA
        $correctOp = DB::connection('sqlsrv')->table('pra')->insertGetId([
            'prop_id' => $newPropId,
            'mlsFNo' => 'RES-2026-2151',
            'fileno' => 'RES-2026-2151',
            'temp_fileno' => 'TEMP-38439',
            'party_1' => 'Kano State Government',
            'party_2' => 'LAWAN HABIBU',
            'Grantor' => 'Kano State Government',
            'Grantee' => 'LAWAN HABIBU',
        ...
        DB::connection('sqlsrv')->table('pra')->where('id', 140942)->update([
            'prop_id' => $newPropId,
