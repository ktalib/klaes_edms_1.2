...
try {
    $result = DB::connection('sqlsrv')->transaction(function() {
        ...
        $newPropId = $service->allocateOrRetrievePropId('RES-2026-2152', 'RES-2026-2152', null, null, ['skip_lookup' => true]);

        // 2. Create the CORRECT OP in PRA
        $correctOp = DB::connection('sqlsrv')->table('pra')->insertGetId([
            'prop_id' => $newPropId,
            'mlsFNo' => 'RES-2026-2152',
            'fileno' => 'RES-2026-2152',
            'temp_fileno' => 'TEMP-38531',
            'party_1' => 'Kano State Government',
            'party_2' => 'ALH SABO ZUB',
            'Grantor' => 'Kano State Government',
            'Grantee' => 'ALH SABO ZUB',
        ...
        DB::connection('sqlsrv')->table('pra')->where('id', 141008)->update([
            'prop_id' => $newPropId,
