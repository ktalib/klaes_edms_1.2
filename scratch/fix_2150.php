...
try {
    $result = DB::connection('sqlsrv')->transaction(function() {
        ...
        $newPropId = $service->allocateOrRetrievePropId('RES-2026-2150', 'RES-2026-2150', null, null, ['skip_lookup' => true]);

        // 2. Create the CORRECT OP in PRA
        $correctOp = DB::connection('sqlsrv')->table('pra')->insertGetId([
            'prop_id' => $newPropId,
            'mlsFNo' => 'RES-2026-2150',
            'fileno' => 'RES-2026-2150',
            'temp_fileno' => 'TEMP-38430',
            'party_1' => 'Kano State Government',
            'party_2' => 'HARUNA MUAZU',
            'Grantor' => 'Kano State Government',
            'Grantee' => 'HARUNA MUAZU',
        ...
        DB::connection('sqlsrv')->table('pra')->where('id', 140941)->update([
            'prop_id' => $newPropId,
