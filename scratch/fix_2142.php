...
try {
    $result = DB::connection('sqlsrv')->transaction(function() {
        ...
        $newPropId = $service->allocateOrRetrievePropId('RES-2026-2142', 'RES-2026-2142', null, null, ['skip_lookup' => true]);

        // 2. Create the CORRECT OP in PRA
        $correctOp = DB::connection('sqlsrv')->table('pra')->insertGetId([
            'prop_id' => $newPropId,
            'mlsFNo' => 'RES-2026-2142',
            'fileno' => 'RES-2026-2142',
            'temp_fileno' => 'TEMP-38226',
            'party_1' => 'Kano State Government',
            'party_2' => 'BAFFAN MASALLACHI',
            'Grantor' => 'Kano State Government',
            'Grantee' => 'BAFFAN MASALLACHI',
        ...
        DB::connection('sqlsrv')->table('pra')->where('id', 140629)->update([
            'prop_id' => $newPropId,
