...
$(document).ready(function() {
    ...
    $(document).off('click', '#generate-betterment-btn').on('click', '#generate-betterment-btn', function() {
        ...
        fetch(`${window.location.origin}/gisedms/betterment-bill/store`, {
            method: 'POST',
            headers: {
    ...
    window.checkForSavedBalanceBill = function() {
        ...
        fetch(`${window.location.origin}/gisedms/sub-final-bill/show/${currentApplication.fileId}`)
            .then(response => response.json())
            .then(data => {
    ...
    window.checkForSavedBettermentBill = function() {
        ...
        fetch(`${window.location.origin}/gisedms/betterment-bill/show/${currentApplication.fileId}`)
            .then(response => response.json())
            .then(data => {
    ...
    $(document).off('click', '#save-balance-bill-btn').on('click', '#save-balance-bill-btn', function() {
        ...
        // Send AJAX request to save the bill
        fetch(`${window.location.origin}/gisedms/sub-final-bill/save`, {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json',
                'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
...
// Additional event listeners to ensure betterment bills are loaded
...
// Periodic check to ensure bills are loaded
