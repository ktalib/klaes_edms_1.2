...
window.openQuickBatchModal = function (selectedInstruments) {
  ...
  if (typeof fetchNextSerialNumber === 'function') {
    ...
    // Determine instrument type from the selected instruments (use the first one)
    const instrumentType = quickBatchInstruments.length > 0 ? quickBatchInstruments[0].instrumentType : null;
    console.log('Using instrument type for serial fetch:', instrumentType);
    ...
    fetchNextSerialNumber(instrumentType)
      ...
      .catch(err => {
        ...
        // fallback to quick-batch specific fetch - also pass type if possible, though that function might need update
        // checking fetchNextSerialNumberForQuickBatch signature... it does NOT take args currently.
        // But since we prefer the centralized one, this fallback is rare. 
...
function fetchNextSerialNumberForQuickBatch() {
  ...
  const instrumentType = quickBatchInstruments.length > 0 ? quickBatchInstruments[0].instrumentType : null;
  let url = `${base}/instrument_registration/get-next-serial`;
  if (instrumentType) {
