...
$(document).on("click", ".common_confirm_dialog", function (e) {
    ...
    if (typeof Swal !== 'undefined') {
        ...
        if (confirm("Are you sure you want to delete " + actions + " ?")) {
            dialogForm.submit();
        }
    }
});
...
function toastrs(title, message, status) {
    ...
    if (typeof notifier !== 'undefined') {
        if (status == "success") {
            notifier.show("Success!", message, "success", typeof successImg !== 'undefined' ? successImg : "", 4000);
        } else {
            notifier.show("Success!", message, "success", typeof errorImg !== 'undefined' ? errorImg : "", 4000);
        }
    } else {
        alert(message);
    }
...
function ckediter(editer_id = "") {
    ...
    if ($(editer_id).length > 0 && typeof ClassicEditor !== 'undefined') {
