/**
 * PHS File Number Modal Plugin
 * Self-contained PHS-portal copy of the internal Global File Number modal.
 * Namespaced as window.PhsFileNoModal against #phs-fileno-modal, and talks to
 * the auth:phs-gated /phs/api/file-numbers/* endpoints so it never depends on
 * (or is affected by) the internal staff-side modal.
 * Supports MLS, KANGIS, New KANGIS + manual builders with live preview.
 */
...
(function ($) {
    ...
    const CONFIG = {
        STORAGE_KEY: 'phsFileno',
        RECENT_LIMIT: 5,
        API_ENDPOINTS: {
            MLS_FILES: '/phs/api/file-numbers/mls',
            KANGIS_FILES: '/phs/api/file-numbers/kangis',
            NEWKANGIS_FILES: '/phs/api/file-numbers/newkangis',
            LOOKUP: '/phs/api/file-numbers/lookup'
        }
    };
    ...
    // Main PhsFileNoModal object
    window.PhsFileNoModal = {
        // Configuration
        ...
        init: function () {
            ...
            console.log('PhsFileNoModal initialized');
        },

        // Open modal with options
        open: function (options = {}) {
            const modal = $('#phs-fileno-modal');
            if (modal.length === 0) {
        ...
        switchTab: function (tabName) {
            if (!tabName) return;
            try { console.log('[PhsFileNoModal] switchTab start', { tabName }); } catch (e) { }
            ...
            try { console.log('[PhsFileNoModal] switchTab complete', { tabName }); } catch (e) { }
        },
        ...
        populateSmartSelector: function (tabName, files) {
            ...
            if (typeof $ !== 'undefined' && $.fn.select2) {
                ...
                selector.select2({
                    ...
                    allowClear: true,
                    width: '100%',
                    dropdownParent: $('#phs-fileno-modal'),
                        ...
                        transport: function (params, success, failure) {
                            ...
                            $.ajax({
                                ...
                                data: {
                                    ...
                                    limit: 20,
                                    exclude_matched: PhsFileNoModal.config.exclude_matched || ''
                                },
                                headers: {
                                    'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content'),
                                    'Content-Type': 'application/json'
                                ...
                                .done(function (response) {
                                    if (response.success && Array.isArray(response.files)) {
                                        let results = response.files.map(file => {
                                            const fileNumber = PhsFileNoModal.extractFileNumber(file, tabName);
                                            return {
                                        ...
                                        const _excludePfxSearch = (PhsFileNoModal.config.excludePrefixes || []).map(p => p.toUpperCase());
        ...
        extractFileNumber: function (file, tabName) {
            switch (tabName) {
                case 'mls':
                    ...
                    if (mlsNum) {
                        console.log('[PhsFileNoModal] Extracted MLS file number:', mlsNum);
                    }
                    return mlsNum;
                case 'kangis':
        ...
        apply: async function () {
            ...
            if (!fileNumber && inputMethod === 'smart') {
                ...
                if (selected) {
                    try {
                        ...
                        console.log('[PhsFileNoModal] Smart selector extracted file number:', fileNumber, 'from data:', fileData);
                    } catch (e) {
            ...
            console.log('[PhsFileNoModal] apply() called', { tabName, inputMethod, fileNumber });

            if (!fileNumber || fileNumber === '' || fileNumber === '-') {
                console.warn('[PhsFileNoModal] No file number, aborting apply');
                return;
            }
            ...
            try {
                ...
                if (this.config.callback && typeof this.config.callback === 'function') {
                    console.log('[PhsFileNoModal] Calling callback with fileNumber:', fileNumber);
                    ...
                    console.warn('[PhsFileNoModal] No callback configured or callback is not a function');
                }
        ...
        resetForm: function () {
            try {
                ...
                $('#phs-fileno-modal input:not([type="radio"]):not([type="checkbox"]):not([type="file"]):not([readonly]), #phs-fileno-modal select').val('').trigger('change');
                $('#phs-fileno-modal input[type="checkbox"]').prop('checked', false).trigger('change');
        ...
        bindEvents: function () {
            ...
            $(document).on('click', '.fileno-tab-btn', function(e) {
                ...
                console.log('[PhsFileNoModal] tab button clicked', tabName);
                if (tabName) {
            ...
            */
            ...
            $(document).on('input change', '#phs-fileno-modal input, #phs-fileno-modal select', function () {
                self.updatePreview();
            });
            ...
            $(document).on('keydown', function (e) {
                if ($('#phs-fileno-modal').hasClass('hidden')) return;
    ...
    $.fn.phsFileNoModal = function (options) {
    ...
    $(document).ready(function () {
        if ($('#phs-fileno-modal').length > 0) {
            PhsFileNoModal.init();

            // Auto-initialize buttons with data-phs-fileno-modal attribute
            $('[data-phs-fileno-modal]').each(function () {
                const $btn = $(this);
                const options = $btn.data('phs-fileno-modal-options') || {};
                $btn.phsFileNoModal(options);
            });
        }
    });
