...
Route::group(['middleware' => ['auth'], 'prefix' => 'instruments'], function () {
    ...
    // Read-only peek at the next registration particulars for the type being captured.
    Route::get('/next-registration-particulars', [App\Http\Controllers\InstrumentController::class, 'nextRegistrationParticulars'])->name('instruments.nextRegistrationParticulars');
...
// Print and preview must not disagree. The on-screen letter reads
// PlanningRecommendationController's assemblers (via planning-tables/*), while
// this route used to re-implement the same assembly inline - and drifted badly:
// it emitted the JSI shared-utilities picklist as utilities (41 rows) and every
// buyer row as a fragmented-unit dimension (230 rows), and it never rejected the
// conversion-only meta sentinels. Delegating keeps one implementation.
Route::get('planning-recommendation/print/{id}', function ($id) {
    return app(App\Http\Controllers\PlanningRecommendationController::class)->printRecommendation($id);
...
Route::group(['middleware' => ['auth', 'XSS'], 'prefix' => 'fileindexing'], function () {
    ...
    Route::get('/leaderboard-data', [FileIndexingController::class, 'leaderboardData'])->name('fileindexing.leaderboard-data');
