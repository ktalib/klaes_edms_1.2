...
-- Add foreign key constraint for generated_by (assuming you have a users table)
-- Uncomment and adjust the table name if you have a users table
-- ALTER TABLE dbo.tracking_sheet 
-- ADD CONSTRAINT FK_tracking_sheet_generated_by 
-- FOREIGN KEY (generated_by) REFERENCES dbo.users(id);

-- Add foreign key constraint for last_printed_by (assuming you have a users table)
-- Uncomment and adjust the table name if you have a users table  
-- ALTER TABLE dbo.tracking_sheet 
-- ADD CONSTRAINT FK_tracking_sheet_last_printed_by 
-- FOREIGN KEY (last_printed_by) REFERENCES dbo.users(id);
GO
...
-- INSERT INTO dbo.tracking_sheet (
--     batch_id, 
--     batch_name, 
--     file_count, 
--     selected_file_ids, 
...
--     status
-- ) VALUES 
-- ('BATCH_001_' + CONVERT(VARCHAR, GETDATE(), 112), 'Batch 100 Files - ' + CONVERT(VARCHAR, GETDATE(), 120), 100, '[1,2,3,4,5]', 1, 'auto_100', 'generated'),
-- ('BATCH_002_' + CONVERT(VARCHAR, GETDATE(), 112), 'Batch 200 Files - ' + CONVERT(VARCHAR, GETDATE(), 120), 200, '[6,7,8,9,10]', 1, 'auto_200', 'generated');

PRINT 'tracking_sheet table created successfully!';
PRINT 'Remember to:';
PRINT '1. Replace [your_database_name] with your actual database name';
PRINT '2. Uncomment foreign key constraints if you have a users table';
PRINT '3. Adjust user table name in foreign key constraints if needed';
GO