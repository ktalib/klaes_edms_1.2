/* ============================================================================
   Register the op_propid_matches migration in the MySQL ledger
   ----------------------------------------------------------------------------
   Companion to database/sql/2026_09_03_create_op_propid_matches.sql —
   ...
   WHAT THIS DOES
   Marks 2026_09_03_000000_create_op_propid_matches_table as run, so a later
...
SELECT
    (SELECT COUNT(*) FROM migrations
      WHERE migration = '2026_09_03_000000_create_op_propid_matches_table') AS ledger_rows_before,
    (SELECT MAX(batch) FROM migrations)                                     AS current_max_batch;

/* Insert, guarded */
INSERT INTO migrations (migration, batch)
SELECT '2026_09_03_000000_create_op_propid_matches_table',
       COALESCE((SELECT MAX(m.batch) FROM (SELECT batch FROM migrations) m), 0) + 1
  FROM DUAL
 WHERE NOT EXISTS (
       SELECT 1 FROM (SELECT migration FROM migrations) x
        WHERE x.migration = '2026_09_03_000000_create_op_propid_matches_table'
 );

/* Verify — expect exactly 1 */
SELECT COUNT(*) AS ledger_rows_after
  FROM migrations
 WHERE migration = '2026_09_03_000000_create_op_propid_matches_table';
