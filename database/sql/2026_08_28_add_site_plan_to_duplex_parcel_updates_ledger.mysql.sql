/* ============================================================================
   Register the duplex_parcel_updates.site_plan migration in the MySQL ledger
   ----------------------------------------------------------------------------
   Companion to database/sql/2026_08_28_add_site_plan_to_duplex_parcel_updates.sql
   ...
   in the MySQL `klas` database while duplex_parcel_updates lives on SQL Server
   (the migration pins ->connection('sqlsrv')). The SQL Server `migrations` table
   is a legacy copy artisan no longer writes to; inserting there registers nothing.
...
SELECT
    (SELECT COUNT(*) FROM migrations
      WHERE migration = '2026_08_28_110000_add_site_plan_to_duplex_parcel_updates_table') AS ledger_rows_before,
    (SELECT MAX(batch) FROM migrations)                                                   AS current_max_batch;

/* Insert, guarded */
INSERT INTO migrations (migration, batch)
SELECT '2026_08_28_110000_add_site_plan_to_duplex_parcel_updates_table',
       COALESCE((SELECT MAX(m.batch) FROM (SELECT batch FROM migrations) m), 0) + 1
  FROM DUAL
 WHERE NOT EXISTS (
       SELECT 1 FROM (SELECT migration FROM migrations) x
        WHERE x.migration = '2026_08_28_110000_add_site_plan_to_duplex_parcel_updates_table'
 );

/* Verify — expect exactly 1 */
SELECT COUNT(*) AS ledger_rows_after
  FROM migrations
 WHERE migration = '2026_08_28_110000_add_site_plan_to_duplex_parcel_updates_table';
