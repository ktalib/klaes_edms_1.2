/* ============================================================================
   Register the conversion_applications acquisition-method migration
   in the MySQL migrations ledger
   ----------------------------------------------------------------------------
   Companion to
     database/sql/2026_08_11_add_acquisition_method_to_conversion_applications.sql
   ...
   in the MySQL `klas` database while `conversion_applications` lives on SQL
...
SELECT
    (SELECT COUNT(*) FROM migrations
      WHERE migration = '2026_08_11_190000_add_acquisition_method_to_conversion_applications') AS ledger_rows_before,
    (SELECT MAX(batch) FROM migrations)                                                        AS current_max_batch;

/* Insert, guarded */
INSERT INTO migrations (migration, batch)
SELECT '2026_08_11_190000_add_acquisition_method_to_conversion_applications',
       COALESCE((SELECT MAX(m.batch) FROM (SELECT batch FROM migrations) m), 0) + 1
  FROM DUAL
 WHERE NOT EXISTS (
       SELECT 1 FROM (SELECT migration FROM migrations) x
        WHERE x.migration = '2026_08_11_190000_add_acquisition_method_to_conversion_applications'
 );

/* Verify — expect exactly 1 */
SELECT COUNT(*) AS ledger_rows_after
  FROM migrations
 WHERE migration = '2026_08_11_190000_add_acquisition_method_to_conversion_applications';
