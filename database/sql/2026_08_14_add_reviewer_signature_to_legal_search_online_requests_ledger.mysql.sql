/* ============================================================================
   Register the reviewer_signature_path migration in the MySQL ledger
   ----------------------------------------------------------------------------
   Companion to
   database/sql/2026_08_14_add_reviewer_signature_to_legal_search_online_requests.sql —
   ...
   WHAT THIS DOES
   Marks 2026_08_14_000001_add_reviewer_signature_to_legal_search_online_requests
...
SELECT
    (SELECT COUNT(*) FROM migrations
      WHERE migration = '2026_08_14_000001_add_reviewer_signature_to_legal_search_online_requests') AS ledger_rows_before,
    (SELECT MAX(batch) FROM migrations)                                                             AS current_max_batch;

/* Insert, guarded */
INSERT INTO migrations (migration, batch)
SELECT '2026_08_14_000001_add_reviewer_signature_to_legal_search_online_requests',
       COALESCE((SELECT MAX(m.batch) FROM (SELECT batch FROM migrations) m), 0) + 1
  FROM DUAL
 WHERE NOT EXISTS (
       SELECT 1 FROM (SELECT migration FROM migrations) x
        WHERE x.migration = '2026_08_14_000001_add_reviewer_signature_to_legal_search_online_requests'
 );

/* Verify — expect exactly 1 */
SELECT COUNT(*) AS ledger_rows_after
  FROM migrations
 WHERE migration = '2026_08_14_000001_add_reviewer_signature_to_legal_search_online_requests';
