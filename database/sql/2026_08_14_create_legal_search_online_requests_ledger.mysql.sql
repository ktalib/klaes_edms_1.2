/* ============================================================================
   Register the legal_search_online_requests migration in the MySQL ledger
   ----------------------------------------------------------------------------
   Companion to
   database/sql/2026_08_14_create_legal_search_online_requests.sql —
   ...
   WHAT THIS DOES
   Marks 2026_08_14_000000_create_legal_search_online_requests_table as run, so
   a later `php artisan migrate` does not attempt it. The migration is guarded
...
SELECT
    (SELECT COUNT(*) FROM migrations
      WHERE migration = '2026_08_14_000000_create_legal_search_online_requests_table') AS ledger_rows_before,
    (SELECT MAX(batch) FROM migrations)                                               AS current_max_batch;

/* Insert, guarded */
INSERT INTO migrations (migration, batch)
SELECT '2026_08_14_000000_create_legal_search_online_requests_table',
       COALESCE((SELECT MAX(m.batch) FROM (SELECT batch FROM migrations) m), 0) + 1
  FROM DUAL
 WHERE NOT EXISTS (
       SELECT 1 FROM (SELECT migration FROM migrations) x
        WHERE x.migration = '2026_08_14_000000_create_legal_search_online_requests_table'
 );

/* Verify — expect exactly 1 */
SELECT COUNT(*) AS ledger_rows_after
  FROM migrations
 WHERE migration = '2026_08_14_000000_create_legal_search_online_requests_table';
