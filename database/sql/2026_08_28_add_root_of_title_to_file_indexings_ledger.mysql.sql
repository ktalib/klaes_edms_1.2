/* ============================================================================
   Register the file_indexings.root_of_title migration in the MySQL ledger
   ----------------------------------------------------------------------------
   Companion to database/sql/2026_08_28_add_root_of_title_to_file_indexings.sql
   ...
   in the MySQL `klas` database while file_indexings lives on SQL Server (the
   migration pins ->connection('sqlsrv')). The SQL Server `migrations` table is a
   legacy copy artisan no longer writes to; inserting there registers nothing.
...
SELECT
    (SELECT COUNT(*) FROM migrations
      WHERE migration = '2026_08_28_100000_add_root_of_title_to_file_indexings_table') AS ledger_rows_before,
    (SELECT MAX(batch) FROM migrations)                                                AS current_max_batch;

/* Insert, guarded */
INSERT INTO migrations (migration, batch)
SELECT '2026_08_28_100000_add_root_of_title_to_file_indexings_table',
       COALESCE((SELECT MAX(m.batch) FROM (SELECT batch FROM migrations) m), 0) + 1
  FROM DUAL
 WHERE NOT EXISTS (
       SELECT 1 FROM (SELECT migration FROM migrations) x
        WHERE x.migration = '2026_08_28_100000_add_root_of_title_to_file_indexings_table'
 );

/* Verify — expect exactly 1 */
SELECT COUNT(*) AS ledger_rows_after
  FROM migrations
 WHERE migration = '2026_08_28_100000_add_root_of_title_to_file_indexings_table';
