/* ============================================================================
   Register the allocation_list_stage existing-allocation migration in the
   MySQL migrations ledger
   ----------------------------------------------------------------------------
   Companion to
     database/sql/2026_08_11_add_existing_allocation_fields_to_allocation_list_stage.sql
   ...
   ledger in the MySQL `klas` database while allocation_list_stage lives on SQL
...
SELECT
    (SELECT COUNT(*) FROM migrations
      WHERE migration = '2026_08_11_150000_add_existing_allocation_fields_to_allocation_list_stage') AS ledger_rows_before,
    (SELECT MAX(batch) FROM migrations)                                                              AS current_max_batch;

/* Insert, guarded */
INSERT INTO migrations (migration, batch)
SELECT '2026_08_11_150000_add_existing_allocation_fields_to_allocation_list_stage',
       COALESCE((SELECT MAX(m.batch) FROM (SELECT batch FROM migrations) m), 0) + 1
  FROM DUAL
 WHERE NOT EXISTS (
       SELECT 1 FROM (SELECT migration FROM migrations) x
        WHERE x.migration = '2026_08_11_150000_add_existing_allocation_fields_to_allocation_list_stage'
 );

/* Verify — expect exactly 1 */
SELECT COUNT(*) AS ledger_rows_after
  FROM migrations
 WHERE migration = '2026_08_11_150000_add_existing_allocation_fields_to_allocation_list_stage';
