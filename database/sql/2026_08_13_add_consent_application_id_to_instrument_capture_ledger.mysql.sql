/* ============================================================================
   Register the instrument_capture.consent_application_id migration
   in the MySQL ledger
   ----------------------------------------------------------------------------
   Companion to
   database/sql/2026_08_13_add_consent_application_id_to_instrument_capture.sql —
   ...
   WHAT THIS DOES
   Marks 2026_08_13_120000_add_consent_application_id_to_instrument_capture as
...
SELECT
    (SELECT COUNT(*) FROM migrations
      WHERE migration = '2026_08_13_120000_add_consent_application_id_to_instrument_capture') AS ledger_rows_before,
    (SELECT MAX(batch) FROM migrations)                                                       AS current_max_batch;

/* Insert, guarded */
INSERT INTO migrations (migration, batch)
SELECT '2026_08_13_120000_add_consent_application_id_to_instrument_capture',
       COALESCE((SELECT MAX(m.batch) FROM (SELECT batch FROM migrations) m), 0) + 1
  FROM DUAL
 WHERE NOT EXISTS (
       SELECT 1 FROM (SELECT migration FROM migrations) x
        WHERE x.migration = '2026_08_13_120000_add_consent_application_id_to_instrument_capture'
 );

/* Verify — expect exactly 1 */
SELECT COUNT(*) AS ledger_rows_after
  FROM migrations
 WHERE migration = '2026_08_13_120000_add_consent_application_id_to_instrument_capture';
