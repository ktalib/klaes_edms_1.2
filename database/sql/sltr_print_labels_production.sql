-- =============================================================================
-- SLTR Print File Labels — Production SQL Script
-- Creates: sltr_print_label_batches, sltr_print_label_batch_items,
--          sltr_rack_shelf_labels (with A1–Z100 seed data = 2600 rows)
-- Target:  SQL Server (sqlsrv)
-- =============================================================================

-- 1. sltr_print_label_batches
IF NOT EXISTS (SELECT * FROM sys.tables WHERE name = 'sltr_print_label_batches')
BEGIN
    CREATE TABLE sltr_print_label_batches (
        id              BIGINT IDENTITY(1,1) PRIMARY KEY,
        batch_number    NVARCHAR(100)  NOT NULL,
        prefix          NVARCHAR(20)   NOT NULL DEFAULT 'SLTR',
    ...
    CREATE UNIQUE INDEX uq_sltr_plb_prefix_sysbatch
        ON sltr_print_label_batches (prefix, sys_batch_no)
        WHERE sys_batch_no IS NOT NULL AND deleted_at IS NULL;
END;
GO

-- 2. sltr_print_label_batch_items
IF NOT EXISTS (SELECT * FROM sys.tables WHERE name = 'sltr_print_label_batch_items')
BEGIN
    CREATE TABLE sltr_print_label_batch_items (
        id              BIGINT IDENTITY(1,1) PRIMARY KEY,
        ...
        created_at      DATETIME2      NULL,
        updated_at      DATETIME2      NULL,
        CONSTRAINT fk_sltr_plbi_batch
            FOREIGN KEY (batch_id) REFERENCES sltr_print_label_batches(id)
            ON DELETE CASCADE
    );
END;
GO

-- 3. sltr_rack_shelf_labels
IF NOT EXISTS (SELECT * FROM sys.tables WHERE name = 'sltr_rack_shelf_labels')
BEGIN
    CREATE TABLE sltr_rack_shelf_labels (
        id          BIGINT IDENTITY(1,1) PRIMARY KEY,
...
-- 4. Seed sltr_rack_shelf_labels with A1–Z100 (2600 rows)
IF (SELECT COUNT(*) FROM sltr_rack_shelf_labels) = 0
BEGIN
    ...
    BEGIN
        ...
        WHILE @shelf <= 100
        BEGIN
            INSERT INTO sltr_rack_shelf_labels (rack, shelf, full_label, is_used, counter, status, created_at)
