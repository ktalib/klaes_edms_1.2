/* ============================================================================
   Register the customer-type migration in the MySQL ledger
   ----------------------------------------------------------------------------
   Companion to
   database/sql/2026_09_02_add_customer_type_to_legal_search_online_verifications.sql —
   ...
   WHAT THIS DOES
   Marks 2026_09_02_000000_add_customer_type_to_legal_search_online_verifications
   as run, so a later `php artisan migrate` does not attempt it. The migration is
   guarded column-by-column and would be a harmless no-op either way — this only
...
SELECT
    (SELECT COUNT(*) FROM migrations
      WHERE migration = '2026_09_02_000000_add_customer_type_to_legal_search_online_verifications') AS ledger_rows_before,
    (SELECT MAX(batch) FROM migrations)                                                             AS current_max_batch;

/* Insert, guarded */
INSERT INTO migrations (migration, batch)
SELECT '2026_09_02_000000_add_customer_type_to_legal_search_online_verifications',
       COALESCE((SELECT MAX(m.batch) FROM (SELECT batch FROM migrations) m), 0) + 1
  FROM DUAL
 WHERE NOT EXISTS (
       SELECT 1 FROM (SELECT migration FROM migrations) x
        WHERE x.migration = '2026_09_02_000000_add_customer_type_to_legal_search_online_verifications'
 );

/* Verify — expect exactly 1 */
SELECT COUNT(*) AS ledger_rows_after
  FROM migrations
 WHERE migration = '2026_09_02_000000_add_customer_type_to_legal_search_online_verifications';
