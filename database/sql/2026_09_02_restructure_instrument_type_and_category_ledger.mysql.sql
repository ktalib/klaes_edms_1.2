/* ============================================================================
   Register the instrument Type/Category restructure migration in the MySQL ledger
   ----------------------------------------------------------------------------
   Companion to
   database/sql/2026_09_02_restructure_instrument_type_and_category.sql —
   ...
   WHAT THIS DOES
   Marks 2026_09_02_120000_restructure_instrument_type_and_category as run, so a later
...
SELECT
    (SELECT COUNT(*) FROM migrations
      WHERE migration = '2026_09_02_120000_restructure_instrument_type_and_category') AS ledger_rows_before,
    (SELECT MAX(batch) FROM migrations)                                   AS current_max_batch;

/* Insert, guarded */
INSERT INTO migrations (migration, batch)
SELECT '2026_09_02_120000_restructure_instrument_type_and_category',
       COALESCE((SELECT MAX(m.batch) FROM (SELECT batch FROM migrations) m), 0) + 1
  FROM DUAL
 WHERE NOT EXISTS (
       SELECT 1 FROM (SELECT migration FROM migrations) x
        WHERE x.migration = '2026_09_02_120000_restructure_instrument_type_and_category'
 );

/* Verify — expect exactly 1 */
SELECT COUNT(*) AS ledger_rows_after
  FROM migrations
 WHERE migration = '2026_09_02_120000_restructure_instrument_type_and_category';
