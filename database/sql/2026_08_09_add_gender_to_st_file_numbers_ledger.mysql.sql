/* ============================================================================
   Register the st_file_numbers.gender migration in the MySQL migrations ledger
   ----------------------------------------------------------------------------
   Companion to database/sql/2026_08_09_add_gender_to_st_file_numbers.sql
...
SELECT
    (SELECT COUNT(*) FROM migrations
      WHERE migration = '2026_08_09_140000_add_gender_to_st_file_numbers_table') AS ledger_rows_before,
    (SELECT MAX(batch) FROM migrations)                                          AS current_max_batch;

/* Insert, guarded */
INSERT INTO migrations (migration, batch)
SELECT '2026_08_09_140000_add_gender_to_st_file_numbers_table',
       COALESCE((SELECT MAX(m.batch) FROM (SELECT batch FROM migrations) m), 0) + 1
  FROM DUAL
 WHERE NOT EXISTS (
       SELECT 1 FROM (SELECT migration FROM migrations) x
        WHERE x.migration = '2026_08_09_140000_add_gender_to_st_file_numbers_table'
 );

/* Verify — expect exactly 1 */
SELECT COUNT(*) AS ledger_rows_after
  FROM migrations
 WHERE migration = '2026_08_09_140000_add_gender_to_st_file_numbers_table';
